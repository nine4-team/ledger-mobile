...
import { createInventoryScopeConfig, getScopeId } from '../../../src/data/scopeConfig';
...
type BISpaceDetailContentProps = {
  spaceId: string;
...
function BISpaceDetailContent({
  spaceId,
  onSpaceNameChange,
  spaceMenuVisible,
  onCloseSpaceMenu,
}: BISpaceDetailContentProps) {
  ...
  const spaceMenuItems: AnchoredMenuItem[] = useMemo(() => {
    const menuItems: AnchoredMenuItem[] = [
      {
        ...
        label: 'Edit',
        icon: 'edit',
        onPress: () => router.push(`/business-inventory/spaces/${spaceId}/edit`),
      },
    ];
    if (canSaveTemplate) {
      menuItems.push({
    ...
    return menuItems;
  }, [canSaveTemplate, handleDelete, handleSaveTemplate, router, spaceId]);
