...
alter table public.spike_operation_results add constraint spike_operation_results_command_type_check check
 (command_type in ('create_client','create_project','archive_project','archive_client','revise_space_checklists','manage_categories','sell_inventory_items','create_expense','edit_expense','create_invoice'));
...
declare c jsonb:=p_command::jsonb; actor text; account text; operation text; fingerprint text;
  ...
  total numeric:=0; currency text; client text;
begin
  ...
  if jsonb_typeof(c) is distinct from 'object' or c->>'contractVersion' is distinct from 'invoice-create-v1'
    or not(c ?& array['operationId','accountId','actorPrincipalId','projectId','clientId','invoiceId','contractVersion','createdAtMs','name','notes','sources'])
    or c-array['operationId','accountId','actorPrincipalId','projectId','clientId','invoiceId','contractVersion','createdAtMs','name','notes','sources']<>'{}'::jsonb
  ...
  if found then
    if row(result.account_id,result.actor_principal_id,result.command_type,result.command_fingerprint)
      is distinct from row(account,actor,'create_invoice'::text,fingerprint) then
  ...
  begin
    ...
    -- Canonical lock ordering is independent of user-visible line order.
    for source in select value from jsonb_array_elements(c->'sources') order by value->>'kind',value->>'sourceId' loop
      ...
      if exists(select 1 from ledger_private.live_invoice_memberships where account_id=account
          and source_kind=source->>'kind' and source_id=source->>'sourceId' and released_at is null) then
        raise exception 'invoice_source_reserved';
      end if;
    ...
    insert into ledger_private.live_invoice_memberships(account_id,invoice_id,source_kind,source_id,position)
      select account,c->>'invoiceId',value->>'kind',value->>'sourceId',(ordinality-1)::integer
  ...
  values(operation,account,actor,'create_invoice','invoice-create-v1',fingerprint,fingerprint,c->>'invoiceId',
    case when failure is null then 'applied' else 'rejected' end,
    case when failure is null then 'invoice_created' end,failure,to_timestamp((c->>'createdAtMs')::bigint/1000.0),received,received,
    (c->>'createdAtMs')::bigint,floor(extract(epoch from received)*1000)::bigint,floor(extract(epoch from received)*1000)::bigint)
  returning * into result;
  return result;
end;
$$;
revoke all on function ledger_private.create_live_invoice(text) from public,anon,authenticated,service_role;
-- No public endpoint until source coverage, security/replay and client integration pass.
