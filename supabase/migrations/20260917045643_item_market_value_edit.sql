-- Extend the existing operation; old v1 requests and receipt fingerprints survive.
create or replace function ledger_private.edit_item_details(p_command text)
returns public.spike_operation_results language plpgsql security definer set search_path='' as $$
declare
  c jsonb:=p_command::jsonb; changes jsonb:=c->'changes'; selected jsonb; market jsonb;
...
begin
  ...
  if jsonb_typeof(c) is distinct from 'object' or not(c ?& required) or c-required<>'{}'
    or c->>'contractVersion' is distinct from (case when changes?'marketValue' then 'item-details-edit-v2' else 'item-details-edit-v1' end)
    ...
    or changes-array['name','sku','notes','status','bookmark','marketValue']<>'{}' then
  ...
  market:=changes->'marketValue';
  if changes?'marketValue' and market<>'null'::jsonb then
    if jsonb_typeof(market) is distinct from 'object'
      or not(market ?& array['minorUnits','currency']) or market-array['minorUnits','currency']<>'{}'
      or jsonb_typeof(market->'minorUnits') is distinct from 'string'
      or jsonb_typeof(market->'currency') is distinct from 'string'
      or market->>'minorUnits' !~ '^(0|[1-9][0-9]*)$'
      or length(market->>'minorUnits')>19 or (market->>'minorUnits')::numeric>9223372036854775807
      or market->>'currency' !~ '^[A-Z]{3}$' then
      raise sqlstate '22023' using message='Invalid market value';
    end if;
  end if;
  ...
  begin
    for selected in select value from jsonb_array_elements(c->'items') order by value->>'itemId' loop
      ...
      update public.spike_items set
        ...
        market_value_minor_units=case when changes?'marketValue' then (market->>'minorUnits')::bigint else item.market_value_minor_units end,
        market_value_currency=case when changes?'marketValue' then market->>'currency' else item.market_value_currency end,
  ...
  values(operation,account,actor,'edit_item_details',c->>'contractVersion',digest,digest,subject,
...
$$;
revoke all on function ledger_private.edit_item_details(text) from public,anon,service_role;
