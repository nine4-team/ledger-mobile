-- One operator-only writer for complete, ordered Expense/Fee source mappings.
-- The old entry point delegates here; its retained retry payload is unchanged.
create function ledger_private.import_invoice_sources(p_invoice jsonb,p_sources jsonb,p_payment jsonb,
...
begin
  ...
  for entry,line in select e.value,l.value from jsonb_array_elements(p_sources) with ordinality e
    ...
    if jsonb_typeof(entry) is distinct from 'object' or not(entry ?& array['record','source_document_id','source_bytes'])
    ...
    if line->>'source_kind'='expense' then
...
language sql security invoker set search_path='' as $$
  select ledger_private.import_invoice_sources(p_invoice,p_expenses,p_payment,p_source_account,p_source_invoice,p_invoice_bytes)
$$;
