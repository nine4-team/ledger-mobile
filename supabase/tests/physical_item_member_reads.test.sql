...
select is((select count(*) from public.spike_transactions where origin='firebase_client_payment'),0::bigint,'Physical read access does not expose client payment facts to an employee');
