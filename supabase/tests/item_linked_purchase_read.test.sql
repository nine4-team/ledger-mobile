...
select is((select count(*) from public.spike_transactions where id like 'purchase-read-%'),2::bigint,'Full member can read linked and standalone payment history');
select is((select count(*) from ledger_private.item_client_payment_connections where transaction_id like 'purchase-read-%'),1::bigint,'Item-specific relationship evidence still identifies only the linked payment');
...
select is((select count(*) from public.spike_transactions where id like 'purchase-read-%'),0::bigint,'Limited member cannot read client Purchase money');
...
set local role authenticated;
select set_config('request.jwt.claims','{"sub":"10000000-0000-0000-0000-000000000001","role":"authenticated"}',true);
select is((select count(*) from public.spike_transactions where id like 'purchase-read-%'),0::bigint,'Same JWT loses client Purchase read on same-count financial downgrade');
reset role;
...
set local role authenticated;
select is((select count(*) from public.spike_transactions where id like 'purchase-read-%'),2::bigint,'Ending placement does not erase authorized payment history');
reset role;
set local role anon;
