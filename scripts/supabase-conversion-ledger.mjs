...
function discoverConfiguration() {
  const candidates = [
    ...
    "supabase/migrations/20260906061012_vendor_suggestion_reference_read.sql",
    "supabase/seed.sql",
    ...
    "supabase/tests/vendor_suggestion_reference_read.test.sql",
    ...
    "scripts/test-local-vendor-suggestion-read.mjs",
...
function isProductSourceSurface(surface) {
  ...
  const targetOrTrackingPath = (value) =>
    ...
    value === "powersync/sync-streams.yaml" ||
...
function main() {
  ...
  if (command === "source-self-test") {
    ...
    for (const targetPath of ["LedgeriOS/Package.swift", "LedgeriOS/Package.resolved", "LedgeriOS/LedgerLocalPaymentImport/main.swift", "powersync/sync-streams.yaml"]) {
    ...
    console.log("Source omission self-tests passed: source omissions rejected; target package/tool/stream edits use implementation checks.");
    return;
  }
