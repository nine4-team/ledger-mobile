...
for (const sql of queries) {
  ...
  assert.equal(sql.match(/subscription\.parameter\('account_id'\)/g)?.length, 1);
}
...
for (const [index, account, principal] of [[0, "account-primary", "principal-restricted"], [1, "account-other", "principal-other"]]) {
  statements.push(`insert into public.spike_items(id,account_id,description,created_by_principal_id) values (${quote(fixtureIDs[0][index])},${quote(account)},'Synthetic physical stream test',${quote(principal)});`);
...
function capture(label, user, account) {
  for (const [index, source] of queries.entries()) {
    ...
    const sql = source.replace("auth.user_id()", `${quote(user)}::uuid`)
      .replace("subscription.parameter('account_id')", quote(account));
...
for (const { label, index, rows } of results) {
  if (!["member", "other-member"].includes(label)) {
    assert.deepEqual(rows, [], `${label}: projection ${index} must deny all rows`);
    continue;
  }
  const fixtureIndex = label === "member" ? 0 : 1;
  ...
  for (const row of rows) {
    ...
    if (index === 2) assert.equal(row.lifecycle, "active");
  }
}
console.log("local-physical-item-stream: actual three SQL projections pass member, cross-Account, other-user, removal and physical-column checks; all fixtures rolled back (not PowerSync engine validation)");
