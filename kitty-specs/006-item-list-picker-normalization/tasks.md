...
**Tests**: Not required — manual testing across both consumers.
...
## Work Package WP01: usePickerMode Hook + SharedItemsList Integration (Priority: P0) 🎯 MVP

**Goal**: Create the `usePickerMode` hook and integrate picker-mode rendering into SharedItemsList. No consumers changed — existing behavior unchanged.
...
- **Dual-path rendering**: Both embedded map and FlatList paths must be updated — miss one and picker mode partially breaks. Prompt includes exact line ranges.
...
**Independent Test**: Space detail picker works identically — tabs, search, single add, bulk add, eligibility checks, "Added" badges all functional.
...
- Tab bar is ~60 lines of JSX — extract from SharedItemPicker's pattern (lines 254-317) and render inline.
...
- **Tab bar styling**: Extract tab JSX directly from SharedItemPicker — styles are inline and self-contained.
...
**Independent Test**: Transaction detail picker works identically — dynamic tabs (suggested/project/outside), bulk add, eligibility, conflict handling all functional.
...
- Conflict handling via `showItemConflictDialog` stays in the handler — no changes needed.
...
- **Conflict dialog flow**: handleAddSelectedItems already handles conflicts — no changes needed, just ensure it's wired correctly.
...
WP01 (hook + integration)
  ...
  └── WP03 (TransactionDetail migration)  ──┤
                                             └── WP04 (cleanup)
```
