...
### Work Package WP01: Consolidate Space Detail Screens (Priority: P1) 🎯 MVP
...
**Independent Test**: Navigate to a space from both business inventory and from a project. Both render identically with the same sections, media gallery, items, and checklists. Edit a space name, add/remove images, manage items — all work in both contexts.
...
### Risks & Mitigations
- **Navigation breaks**: Both route files remain in place — only content moves to shared component
...
### Parallel Opportunities
- None within this WP — changes are sequential to the same file.
...
- No sticky sections needed — set `stickySectionHeadersEnabled={false}` (item detail has no items list control bar)
...
- filteredAndSortedItems uses useMemo: filter → search → sort pipeline
...
- Hook is pure state management — no rendering, no service calls
...
- Control bar rendering stays in renderSectionHeader (sticky) — ItemsSection provides the items list content only
...
- [x] T030 Verify MediaGallerySection integration on space detail (no image cap regressions, add/remove/set-primary work)
...
- **Sequence**: WP01 → WP02 → WP05 (space path); WP04 → WP05 (hook path)
- **Parallelization**: WP01 ∥ WP03 ∥ WP04 ∥ WP06 can all start simultaneously
