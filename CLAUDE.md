...
Key invariants:
- **Each inventory movement is a new immutable transaction.** No long-lived aggregators. Inventory → project writes a `Purchase` from inventory. Project → inventory writes either a `Return` when the item came from inventory, or a `Sale` only when the business is acquiring a project-originated item into inventory. Accounting shape fields (`amountCents`, `budgetCategoryId`, `type`, `source`, `projectId`) are locked after creation by Firestore security rules. Active membership `itemIds` remains mutable, as do `notes`, `status`, and `updatedAt`.
