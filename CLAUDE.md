...
Key invariants:
- **Each inventory movement is a new structurally immutable transaction.** No long-lived aggregators. Inventory → project writes a `Purchase` from inventory. Project → inventory writes either a `Return` when the item came from inventory, or a `Sale` only when the business is acquiring a project-originated item into inventory. Structural fields (`budgetCategoryId`, `type`, `source`, `projectId`) are locked after creation. Clients cannot edit movement totals directly; the trusted item-price trigger may maintain `amountCents`/`subtotalCents` on an unpaid project-side Purchase-from-Inventory when one of its attached sold items is repriced. Active membership `itemIds` remains mutable, as do `notes`, `status`, and `updatedAt`.
...
- **Project → inventory pricing is origin-aware.** Inventory-originated items go home through a Return that reverses normalized project price. Project-originated items enter inventory through a Sale-to-Inventory at purchase cost; a malformed higher project price must never make the business overpay. Project → project uses the same origin-aware rule for its source exit and normalized project price for the destination Purchase.
