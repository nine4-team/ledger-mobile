...
export type ProjectPriceContribution = {
...
type TransactionData = Record<string, unknown>;

export function isPaidInvoiceForItem(
  invoice: Record<string, unknown>,
  projectId: string,
  itemId: string
): boolean {
  const status = typeof invoice.status === 'string' ? invoice.status.trim().toLowerCase() : '';
  if (status !== 'paid' || invoice.projectId !== projectId) return false;

  const flatItemIds = Array.isArray(invoice.itemIds) ? invoice.itemIds : [];
  if (flatItemIds.includes(itemId)) return true;

  const legacyLines = Array.isArray(invoice.lines) ? invoice.lines : [];
  return legacyLines.some((line: unknown) => line != null &&
    typeof line === 'object' &&
    (line as Record<string, unknown>).sourceType === 'item' &&
    (line as Record<string, unknown>).sourceId === itemId
  );
}

export function hasStableProjectTransactionAssociation(
...
function nonnegativeTaxRate(value: unknown): number {
  if (typeof value !== 'number' || !Number.isFinite(value) || value <= 0) return 0;
  return value;
}

export function itemProjectPriceContribution(data: PriceData): ProjectPriceContribution {
  const priceCents = Math.max(
    nonnegativeCents(data.purchasePriceCents),
    nonnegativeCents(data.projectPriceCents)
  );
...
export function projectPriceChange(before: PriceData, after: PriceData): ProjectPriceChange {
 ...
 */
export function isProjectInventoryPurchase(data: TransactionData): boolean {
...
function adjustedNumber(value: unknown, delta: number): number | undefined {
 ...
 */
export function adjustInventoryPurchaseTotals(
  transaction: TransactionData,
  change: ProjectPriceChange,
  itemIsActiveMember = true
): {
  amountCents: number;
  subtotalCents?: number;
  isComplete?: unknown;
  ...
  if (nextSubtotal != null) result.subtotalCents = nextSubtotal;
  if ('isComplete' in transaction) result.isComplete = transaction.isComplete;

  if (transaction.audit && typeof transaction.audit === 'object' && !Array.isArray(transaction.audit)) {
    const audit = { ...(transaction.audit as Record<string, unknown>) };
    ...
    if (resolvedSubtotal != null) audit.resolvedSubtotalCents = resolvedSubtotal;
    if (itemsSum != null) audit.itemsSumCents = itemsSum;
    if (linkedItemsSum != null) audit.linkedItemsSumCents = linkedItemsSum;
    ...
    if (resolvedSubtotal != null && resolvedSubtotal > 0 && itemsSum != null) {
      ...
      audit.variancePercent = Math.round((variance / resolvedSubtotal) * 10_000) / 100;
      result.isComplete = Math.abs(audit.variancePercent as number) <= 1;
    }
    result.audit = audit;
  }
