...
When this provenance is available, the inventory action menu exposes both **Return to Project** and **Sell**. Return opens the locked reversal above; Sell opens the normal destination picker so the item can be sold to a different project.
...
When "Return to Project" is available, "Sell" remains available as a separate action so the user can choose another project instead of reversing the original sale.
