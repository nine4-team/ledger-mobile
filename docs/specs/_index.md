...
- [modify] [Inventory Movement Transactions](sale-transactions.md) — **Active; category reclassification implemented locally, production rollout pending.** Per-batch inventory movements are origin-aware: inventory → project writes a Purchase at project price; an inventory-originated item returns at its project charge; a project-originated item is acquired into inventory at purchase cost. Project → project applies that rule to the source exit and project price to the destination Purchase. The trusted item-price trigger may adjust an eligible Purchase amount/subtotal until the existing paid-invoice freeze boundary. A dedicated Change Category action allows the entire Purchase to move to another project-enabled itemized category while atomically updating current items; generic movement edits remain blocked. `itemIds` tracks current active membership. Replaces the canonical-sale aggregator model.
