...
**Draft snapshot rule (amended 2026-04-21).** Draft invoices are live previews: the document stores only `itemIds` / `transactionIds` while `status == .draft`. `lines` and `totalCents` are written exactly once, at the `markSent` transition, computed from the then-current item / transaction state. This replaces the earlier behavior in which the picker wrote signed lines onto the draft at create / update time — that approach silently froze draft amounts at save-time and drifted when an item's `projectPriceCents` changed outside the invoice flow. Readers (picker total, invoice detail, PDF builder, InvoiceRow on the Billing subtab) fall through to a live-derivation path whenever `invoice.lines == nil`.
...
**Amendment to Tasks 1.4 / 1.5 (2026-04-21): drafts stopped storing `lines` / `totalCents`.** Initial Phase 1 shipped the picker writing signed lines onto drafts at create / update time. That made draft amounts a stale snapshot of item prices at save-time — editing an item's `projectPriceCents` afterwards left the draft showing the old number until the user re-opened and re-saved. Fix: `InvoiceService.createInvoice` and `updateSelections` now write only `itemIds` / `transactionIds` on drafts (and `updateSelections` emits `FieldValue.delete()` for any pre-existing `lines` / `totalCents` left behind by the prior behavior, so readers fall through to live derivation). `InvoiceService.markSent` now takes `lines: [InvoiceLine]` and `totalCents: Int` and writes them atomically with the status transition and `dateSent`. `InvoiceDetailView.performMarkSent` is the caller that builds the signed lines from `projectContext.items` / `projectContext.transactions`. The `ReportAggregationCalculations.computeInvoiceReport` fallback-when-no-lines branch was upgraded to split charges vs credits by transaction shape instead of dumping everything into charges, and `FinancesTabView.InvoiceRow` now computes the displayed total from live membership when the invoice is a draft. No backfill needed for existing drafts — `updateSelections` clears stale snapshot fields on the next edit, and a draft that is sent without a further edit materializes a fresh snapshot at that moment anyway.
