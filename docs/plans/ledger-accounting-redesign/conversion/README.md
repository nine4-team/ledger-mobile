...
direct Space-create provider is deliberately DRAFT: eleven comment-only leaves,
one pgTAP runner placeholder with no Space assertion, and a complete cross-layer
test matrix are recorded, but independent authority
...
Initial DRAFT commit `2cf4bdb2` passed conversion and target jobs but failed the
...
The Space core-details PowerSync provider is implemented locally across eight
claimed target leaves. Separate forced-RLS detail/checklist relations, one exact
signed active/archived Space stream, causal encrypted reconstruction,
close-aware runtime ownership, a Core-only presenter and isolated staging detail
now execute. Independent review found and drove corrections for an unapproved
timestamp-order constraint, incomplete privilege proof, missing encrypted
restart/startup/invalidation coverage and incomplete-progress rendering. The
...
cutover authority exists; `EVID-SPACE-CORE-DETAILS-PROVIDER-001`.
...
The Client archive browser slice is implemented locally from exact green READY
commit `7187fa1e` / immutable run `33968952286`. Nine executable READY leaves
plus runnable pgTAP CONFIG-86F1E734BC70 now provide generic Account-bound
identity, encrypted offline archive optimism/reconciliation, dependency-aware
FIFO upload, principal-bound shared Client projection, exact pre-archive Project
setup replay with fail-closed new admission, Core-only browser orchestration,
scoped-user RPC and auth-first Postgres handling. The byte-identical inert marker
CONFIG-2DBC5A626444 is retired in place. Root and independent executable review
closed strict terminal/linkage, missing-overlay admission, negative timestamp,
principal binding, canonical numeric parsing, test-matrix and sensitive-logging
