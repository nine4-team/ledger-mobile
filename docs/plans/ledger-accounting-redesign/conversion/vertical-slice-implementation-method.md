...
| macOS XCTest unit bundle without desktop launch | After building changed sources with `xcodebuild ... build-for-testing`, use `xcrun xctest -XCTest <ClassName> <built-unit-test-bundle.xctest>`. Resolve the actual bundle from build output. This runs XCTest cases without Xcode's desktop launcher; it is not UI automation. Require named executed cases: Swift Testing-only selections can exit0 with zero tests through this route. Do not convert existing suites wholesale or count unsupported-device skips as passes. |
