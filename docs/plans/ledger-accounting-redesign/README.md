...
| [Account Discovery and Workspace Selection](../../specs/account-discovery-and-workspace-selection.md) | Visibility-safe Account discovery, deterministic remembered ordering, and explicit local workspace-selection intent | Provider-independent selection contract approved; Auth, offline lease, activation adapters, and hosted authorization remain open |
| [Session Ending and Pending Local Work](../../specs/session-ending-pending-work.md) | Loss-preventing clean, sync-first, cancel, and exact-confirmed destructive local session disposition | Safety contract approved; Auth provider, offline lease, final UI copy, and physical adapters remain open |
