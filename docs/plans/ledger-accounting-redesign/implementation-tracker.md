...
| G0.75 — Target mapping | design | 388 of 555 target-relevant surfaces are target-mapped or later; all 167 residual surfaces have explicit decision/spike/production-evidence blockers; `EVID-M2-WHOLE-MANIFEST-001` plus bounded slice evidence |
...
| Map stable target responsibilities | done | 388 target mappings or later across source and target implementation surfaces; zero incomplete mapped records; 167 unresolved surfaces remain tied to 44 explicit blockers rather than guessed |
...
| Define direct Space creation form-to-application use case | ready | Four conversion surfaces converge current name validation on the canonical target values while only two target-only leaves may change. Caller-supplied raw name/notes become typed intent; the application use case adds exact operation metadata, assembles the verified command, calls `SpaceCreating` exactly once after validation and validates its receipt. The complete local ready gate and two independent corrected-diff reviews pass. Exact-ready-SHA CI remains before implementation; no SwiftUI, physical persistence, optimistic row, authorization, provider/schema/RLS/Sync, app/MCP, migration or production behavior; `EVID-DIRECT-SPACE-CREATION-USE-CASE-001` |
