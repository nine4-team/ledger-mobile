...
| G3 — Implementation | in progress | Decision-independent foundations are active; 107 `LedgerTargetCore` production/test leaf surfaces are verified across completed provider-free slices, including the Project-archive-use-case leaves at commit `232acf92` / run `33758116150`. Two additional comment-only Client-archive-use-case leaves are ready but not implementation-advanced. This count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Define selected-Client archive application use case | ready | Exactly two comment-only target leaves freeze a transient exact Account/Client/expected-revision intent and application orchestration over the verified Client archive operation: caller operation metadata, construction before one port call, receipt validation, structured cancellation, normalized failure preservation and bounded raw errors. Client reads/UI, lifecycle eligibility/projection, physical persistence, authorization, schema/RLS/Sync/provider, app/MCP, migration, hosted resources, production and cutover remain excluded. The complete local READY gate passes all 261 tests, controls, repeatable generation and both staging builds; corrected authority/continuity reviews return GO and immutable exact-ready-SHA CI remains required before executable implementation; `EVID-CLIENT-ARCHIVE-USE-CASE-001` |
