...
| G3 — Implementation | in progress | Decision-independent foundations are active; 82 target surfaces are verified across completed provider-free slices and two session-ending surfaces are implemented locally awaiting exact-commit CI. The count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Define session-ending pending-work safety boundary | verify | Exactly two target surfaces implement environment/Principal/Account-scoped queued/applying/unresolved-rejected/unverified-attachment counts, canonical summary/request fingerprints, clean/sync-first/exact-confirmed destructive dispositions, cancellation no-call, stale/regressed-summary refusal, stable diagnostics and one narrow `AccountSessionEnding` port. Exact ready commit `caa9a900` / run `33662071456` and four focused/all 168 local tests plus both builds pass; exact implementation-commit CI remains. Auth provider/offline lease/UI copy, physical synchronization/cleanup, O-023 retention, schema/RLS/Sync, app/MCP, migration and production remain excluded; `EVID-SESSION-ENDING-POLICY-001` |
