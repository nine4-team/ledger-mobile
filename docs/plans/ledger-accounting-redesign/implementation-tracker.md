...
| Define provider-free Project rename operation | ready | Exactly two comment-only target core/test surfaces and a complete dossier define one stable Account/Project/name/expected-revision intent, shared operation lifecycle, canonical restart and atomic rebound refusal. Behavioral implementation has not started. No Project row/downstream projection, Client/description/category/media/history/lifecycle change, physical persistence, schema/RLS/Sync/provider, app/MCP or migration behavior exists; O-024/O-025 remain open; `EVID-PROJECT-RENAME-001` |
