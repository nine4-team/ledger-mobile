...
| G3 — Implementation | in progress | Decision-independent foundations are active; 119 `LedgerTargetCore` production/test leaf surfaces are verified across completed provider-free slices, including Client rename use-case exact implementation `0f000b8e` / immutable run `33821354656`. Two additional Item-to-Space assignment use-case leaves are locally `implemented` and are not counted as verified until exact implementation-commit CI passes. This count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Define Item-to-Space local-destination-to-application use case | implemented | Exactly two target leaves implement a public transient Account/scope/destination/typed-Item intent and one application boundary joining the verified Space destination directory to the verified assignment operation. It validates Account/scope, resolves only a represented stable SpaceID across all six Project/Inventory × ready/partial/stale combinations, derives its revision from that row, constructs before exactly one assigner call and validates afterward. Missing rows refuse as not represented. Pre-port application failures, operation failures and cancellation remain exact; other port errors—including an impossible application-layer failure—are bounded. Exact READY `d51ea1d3` / run `33827181145`, seven focused/all 309 tests and two corrected-diff reviews pass locally. Clear/no-op, Item selection UI/read, archive, media/marker, scope movement, accounting/provenance, persistence, authorization, provider/schema/RLS/Sync, app/MCP, migration, hosted and production behavior stay excluded. Exact implementation-commit CI remains required for verification; `EVID-ITEM-SPACE-ASSIGNMENT-USE-CASE-001` |
