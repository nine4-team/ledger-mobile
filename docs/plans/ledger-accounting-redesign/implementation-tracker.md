...
| G0.75 — Target mapping | design | 301 of 465 target-relevant surfaces are target-mapped or later; all 164 residual surfaces have explicit decision/spike/production-evidence blockers; `EVID-M2-WHOLE-MANIFEST-001` plus bounded slice evidence |
...
| G3 — Implementation | in progress | Decision-independent environment foundations are active; 28 target surfaces are verified across operation, catalog, shared-list, scoped-route, typed-edit, telemetry, release-manifest, protected-artifact, migration-run-integrity, and operational-health slices. The count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Define deterministic target reference test support | in progress | Separate provider-free package target implements validated synthetic environment/Principal/Account context, finite clock/ID/revision schedules, immutable scripted operation/readiness/failure adapters, canonical restart evidence and production/credential/cross-Account/tamper refusal. Four focused/63 total tests, graph/contracts and both builds pass locally; exact-commit CI remains. Current Swift/MCP helper surfaces stay target-mapped; `EVID-DETERMINISTIC-TEST-SUPPORT-001` |
