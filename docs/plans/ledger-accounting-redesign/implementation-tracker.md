...
| G0.75 — Target mapping | design | 395 of 569 target-relevant surfaces are target-mapped or later; all 174 residual surfaces have explicit decision/spike/production-evidence blockers across 45 validated blocker keys; `EVID-M2-WHOLE-MANIFEST-001` plus bounded slice evidence |
...
| G3 — Implementation | in progress | Decision-independent foundations are active; 115 `LedgerTargetCore` production/test leaf surfaces are verified across completed provider-free slices, including the Project-details-update-use-case leaves at exact implementation commit `55baae9d` / immutable run `33799751617`. Two additional comment-only Project-rename-use-case leaves are `target_mapped` under a `ready` dossier and are not counted as verified. This count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Define typed Project rename intent-to-application use case | ready | Two comment-only target leaves freeze a public non-Codable exact Account/Project/revision/`ProjectDisplayName` intent and a generic application boundary over the verified Project rename operation. The future use case constructs before one `ProjectRenaming.rename` call, validates the receipt afterward, preserves all 12 typed failures and cancellation, and bounds unknown errors. It accepts no raw String and adds no normalization, read/readiness/lifecycle/no-op/UI, broader mutation, persistence, authorization, provider/schema/RLS/Sync, app/MCP, migration, hosted or production behavior. ProjectDetailView, ProjectDetailContainer and the other named source surfaces remain unadvanced. The complete local READY gate passes and both independent corrected-diff reviewers return GO; immutable exact-READY-SHA CI remains before implementation; `EVID-PROJECT-RENAME-USE-CASE-001` |
