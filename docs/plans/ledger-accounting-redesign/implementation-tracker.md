...
| Define provider-free Project-note reads | ready | Exactly two comment-only target core/test surfaces freeze stable Account/Project/note identity, visible-content versus tombstone evidence, immutable creation/source plus revision/edit audit, bounded newest-first `(createdAt,id)` order, structured continuation, explicit complete/incomplete local history and one narrow read port. No mutation authorization, physical persistence, schema/RLS/Sync/provider, app/MCP/search, migration or production behavior exists; `EVID-PROJECT-NOTE-READ-001` |
