...
| Define provider-free Item-to-Space assignment operation | ready | Exactly two comment-only target surfaces freeze one nonempty canonical same-scope `AssignItemsToSpace` intent with stable destination Space/Item identities, exact Space/Item revisions, deterministic active/revision/scope preconditions, atomic refusal, shared operation lifecycle and one narrow port. All 172 existing target tests in 40 suites, graph/contracts, both builds and local controls pass; exact ready-commit CI remains. O-037 stays open because archive is absent; clear assignment, Item scope/accounting mutation, physical persistence, schema/RLS/Sync/provider behavior, app/MCP, migration and production remain excluded; `EVID-ITEM-SPACE-ASSIGNMENT-001` |
