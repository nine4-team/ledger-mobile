...
| Define provider-free vendor-suggestion reference reads | ready | Exactly two comment-only target core/test surfaces freeze stable Account-scoped suggestion identity, preserved bounded display spelling, normalized duplicate detection, active/archive lifecycle, revision, unique order, explicit ready/partial/stale/authoritative-empty local evidence, source-text-only selection and one narrow query port. All five executable obligations remain planned. No O-026 mutation authority, default seeding, canonical Vendor identity, physical persistence, schema/RLS/Sync/provider, app/MCP, migration or production behavior exists; `EVID-VENDOR-SUGGESTION-REFERENCE-001` |
