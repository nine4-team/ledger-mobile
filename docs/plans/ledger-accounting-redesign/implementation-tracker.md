...
| G0.75 — Target mapping | design | 390 of 557 target-relevant surfaces are target-mapped or later; all 167 residual surfaces have explicit decision/spike/production-evidence blockers; `EVID-M2-WHOLE-MANIFEST-001` plus bounded slice evidence |
...
| G3 — Implementation | in progress | Decision-independent foundations are active; 103 `LedgerTargetCore` production/test leaf surfaces are verified across completed provider-free slices, including the direct-Space-use-case leaves at exact implementation commit `c35f1cef` / Actions run `33746648677`. This count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Map stable target responsibilities | done | 390 target mappings or later across source and target implementation surfaces; zero incomplete mapped records; 167 unresolved surfaces remain tied to 44 explicit blockers rather than guessed |
...
| Define Space details update form-to-application use case | in progress | Exactly two target leaves now implement caller-supplied Account/Space/revision and raw name/notes, canonical complete-replacement validation, application-owned operation metadata/command dispatch, one post-validation `SpaceDetailsUpdating` call, receipt validation, Swift cancellation and bounded transport failures. The source modal remains `target_mapped`: initial population, transient UI, dirty/no-op and lifecycle eligibility policy, error/dismissal UX, layout/copy and composition remain outside. Two independent corrected final reviews and the complete local integration gate pass; immutable exact-implementation-SHA CI remains required before verification. No physical persistence, authorization, provider/schema/RLS/Sync, app/MCP, migration or production behavior; `EVID-SPACE-DETAILS-UPDATE-USE-CASE-001` |
