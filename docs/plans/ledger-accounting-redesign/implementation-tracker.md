...
| G0.75 — Target mapping | design | 391 of 575 target-relevant surfaces are target-mapped or later; all 184 residual surfaces have explicit decision/spike/production-evidence blockers across 46 validated blocker keys; `EVID-M2-WHOLE-MANIFEST-001` plus bounded slice evidence |
...
| G3 — Implementation | in progress | Decision-independent foundations are active; 123 `LedgerTargetCore` production/test leaf surfaces are verified across completed provider-free slices, including Item Space-clearing use-case synchronized implemented checkpoint `8181512d` / immutable run `33833543735`. This count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Map stable target responsibilities | done | 387 target mappings or later across source and target implementation surfaces; zero incomplete mapped records; 184 unresolved surfaces remain tied to 46 explicit blockers rather than guessed |
| Generate exact residual decision queue | done | 184 residual surfaces grouped under 46 validated blockers; `npm run conversion:residuals:check` |
| Draft product decision packets | done | Nineteen proposed packets cover all 38 product blockers, including O-039 Project-note text validation and O-040 Project budget pinning; O-021 is UI-only; none is approved by documentation alone |
...
| Freeze complete target-query logical-authority crosswalk | ready | Reviewed all 18 generated TQUERY methods into one closed JSON registry bound to the inventory digest and exact signature hashes. Each row records only reviewed logical scope/result/order/pagination/readiness/authorization, authority references, proposed architecture data-domain dependencies and explicit blocked/undefined axes. Item sections have exact section/upstream order while physical storage order remains undefined; budget contribution-source eligibility/taxonomy is explicitly not defined and contains no O-007/O-015 blocker; unresolved Operations include only queued/applying/rejected and transient failure requeues. O-040 blocks Project preference; O-026 narrowly blocks exact reference-data visibility/download/capability; O-007/O-015 remain only on Item occurrence/provenance persistence; Account identity and multi-domain Operation uncertainty remain explicit. Every row cites its owning verified query-contract evidence. Postgres and local physical planes are globally deferred under proposed A-003/A-004 with no TINDEX IDs, SQL, RLS, Sync assignment or provider/runtime claim. Exactly two comment-only control/test scaffolds are `target_mapped`; both are path-excluded from generic Firebase audit/repair discovery, and the test is exact-path-excluded from current-query candidate inspection, so contiguous forbidden-key sentinels prove both boundaries while all 386 QUERY occurrences remain unchanged. Future validation is closed-schema/exact-structural only with no free-text or Markdown semantic/status parsing. Executable JSON validation, generated output and package/CI hooks await immutable READY approval. Reconciliation of all 386 source `QUERY-*` occurrences is a separate later slice; `EVID-TARGET-QUERY-LOGICAL-AUTHORITY-001` |
...
| Define Project setup selection-to-application use case | done | Exactly two target leaves implement existing ProjectSetupFormSelection/current ProjectSetupFormPreparation command derivation before one ProjectSetupOperating.create call, exact receipt/local-state validation, both typed failure families, cancellation and bounded raw errors. Matching represented ready/partial/stale evidence remains admissible without a new readiness gate; preparation evidence never enters the command. Exact READY commit `863f6fce` / run `33791245215` and exact implementation commit `d624c2e0` / immutable run `33793469069` pass. Root every-line review and six focused/all 279 tests pass; two independent final reviewers returned GO after correcting nil-description, same-identity Client-kind and allocation-order proof gaps. NewProjectView, ProjectFormValidation/tests and ProjectService remain unadvanced; UI/default/category mutation/media, physical persistence, authorization, provider/schema/RLS/Sync, app/MCP, migration and production remain excluded; O-023/O-024/O-025/O-026 stay open; `EVID-PROJECT-SETUP-USE-CASE-001` |
