...
| Enforce complete target query-port method inventory | implemented | Dependency-free lexical scanner, 20-test adversarial suite, deterministic JSON/Markdown, exact package commands and Linux-before-macOS CI dependency now fail closed on stale, malformed, ambiguous or unsupported target query-port declarations. Review caught and the implementation corrected nested string/regex declaration leakage including comment-separated regex context, split multiline-return truncation, incomplete declaration/header/generic/parameter/tail validation including nested component endpoints, and Unicode/backticked-name omission. Reviewed baseline remains 16 verified owners / 16 protocols / 18 methods, all current `watch*` observations, with stable TQUERY identity and separate signature drift; local `TQUERYCONTROL-TEST-001` through `-007` pass and `-008` awaits immutable implementation CI. Later slices own every-TQUERY logical access/index review and reconciliation to all 386 source `QUERY-*` occurrences. No product/query semantics, physical index, provider, hosted or production behavior advances; `EVID-TARGET-QUERY-PORT-INVENTORY-001` |
