...
| G0.75 — Target mapping | design | 387 of 571 target-relevant surfaces are target-mapped or later; all 184 residual surfaces have explicit decision/spike/production-evidence blockers across 46 validated blocker keys; `EVID-M2-WHOLE-MANIFEST-001` plus bounded slice evidence |
...
| G3 — Implementation | in progress | Decision-independent foundations are active; 119 `LedgerTargetCore` production/test leaf surfaces are verified across completed provider-free slices, including Client rename use-case exact implementation `0f000b8e` / immutable run `33821354656`. This count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
