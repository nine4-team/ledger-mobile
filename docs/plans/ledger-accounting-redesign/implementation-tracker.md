...
| G3 — Implementation | in progress | Decision-independent foundations are active; 115 `LedgerTargetCore` production/test leaf surfaces are verified across completed provider-free slices, including the Project-details-update-use-case leaves at exact implementation commit `55baae9d` / immutable run `33799751617`. This count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
