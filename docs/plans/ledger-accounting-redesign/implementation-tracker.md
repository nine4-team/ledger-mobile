...
| Define provider-free Space details update operation | verify | Exactly two target core/test surfaces implement complete normalized name/optional-notes replacement, stable Space identity, exact same-Space expected revision, canonical restart/refusal, shared operation lifecycle and one narrow port. Four focused/all 160 target tests, graph/contracts and both builds pass locally; exact-implementation-commit CI remains. Scope/checklist/template/media/Item/review/lifecycle/accounting mutation, authorization, physical persistence, schema/RLS/Sync/provider behavior, app/MCP, migration and production remain excluded; O-023/O-026/O-037 stay open; `EVID-SPACE-DETAILS-UPDATE-001` |
