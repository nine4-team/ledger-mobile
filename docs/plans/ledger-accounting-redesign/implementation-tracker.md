...
| G0.75 — Target mapping | design | 389 of 573 target-relevant surfaces are target-mapped or later; all 184 residual surfaces have explicit decision/spike/production-evidence blockers across 46 validated blocker keys; `EVID-M2-WHOLE-MANIFEST-001` plus bounded slice evidence |
...
| G3 — Implementation | in progress | Decision-independent foundations are active; 119 `LedgerTargetCore` production/test leaf surfaces are verified across completed provider-free slices, including Client rename use-case exact implementation `0f000b8e` / immutable run `33821354656`. Two additional comment-only Item-to-Space assignment use-case leaves are `target_mapped` under a `ready` dossier and are not counted as verified. This count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Define Item-to-Space local-destination-to-application use case | ready | Exactly two comment-only target leaves freeze a public transient Account/scope/destination/typed-Item intent and one application boundary joining the verified Space destination directory to the verified assignment operation. Future implementation must validate exact Account/scope, resolve only a represented stable SpaceID from ready/partial/stale evidence, derive its revision from that row, construct before exactly one assigner call, validate afterward, preserve all typed failures/cancellation and bound unknown errors. Missing rows refuse as not represented without asserting authoritative nonexistence. Ten obligations require ordinary import, literal reciprocal non-mirrored ownership and exact allowlists. Clear/no-op, Item selection UI/read, archive, media/marker, scope movement, accounting/provenance, persistence, authorization, provider/schema/RLS/Sync, app/MCP, migration, hosted and production behavior stay excluded. Exact READY commit and immutable CI remain required; `EVID-ITEM-SPACE-ASSIGNMENT-USE-CASE-001` |
