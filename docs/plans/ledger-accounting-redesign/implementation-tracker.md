...
| Define Space details update form-to-application use case | ready | Exactly two comment-only target leaves freeze caller-supplied Account/Space/revision and raw name/notes, canonical complete-replacement validation, application-owned operation metadata/command dispatch, one post-validation `SpaceDetailsUpdating` call, receipt validation, Swift cancellation and bounded transport failures. The source modal remains `target_mapped`: initial population, transient UI, dirty/no-op and lifecycle eligibility policy, error/dismissal UX, layout/copy and composition remain outside. Two independent corrected actual-diff reviews and the complete local ready gate pass; immutable exact-ready-SHA CI remains required before implementation. No physical persistence, authorization, provider/schema/RLS/Sync, app/MCP, migration or production behavior; `EVID-SPACE-DETAILS-UPDATE-USE-CASE-001` |
