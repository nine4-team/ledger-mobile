...
| G3 — Implementation | in progress | Decision-independent foundations are active; 101 `LedgerTargetCore` production/test leaf surfaces are verified across completed provider-free slices, with two more direct-Space-use-case leaves implemented locally and awaiting exact-commit CI. This count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
