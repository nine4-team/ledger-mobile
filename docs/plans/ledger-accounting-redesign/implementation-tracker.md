...
| G3 — Implementation | in progress | Decision-independent foundations are active; 121 `LedgerTargetCore` production/test leaf surfaces are verified across completed provider-free slices, including Item-to-Space assignment use-case exact implementation `04331950` / immutable run `33828908233`. Two additional comment-only Item Space-clearing use-case leaves are `target_mapped` under a `ready` dossier and are not counted as verified. This count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Define Item Space-clear intent-to-application use case | ready | Exactly two comment-only target leaves freeze a public transient Account/scope/typed-Item-revision-current-Space intent and one application boundary above the verified clear operation. Future implementation must bind the exact caller-supplied conflict values through draft/command construction before exactly one clearer call, validate afterward, preserve all 15 typed failures and cancellation, and bound only unknown port errors. Current-Space/scope claims remain untrusted until authoritative apply; stale assigned-looking evidence may dispatch and later conflict. Nil/no-current-Space intent is structurally unrepresentable, empty rejects, and no synthetic no-op receipt exists. Ten obligations freeze literal reciprocal ownership, exact dependency/source-status preservation and distinct READY/implementation/promotion allowlists. Read/UI eligibility, archive/O-037, attachment references/bytes/O-023, marker mutation, destination/scope/accounting/provenance, persistence, authorization, provider/schema/RLS/Sync, app/MCP, migration, hosted and production behavior stay excluded. Exact READY commit and immutable CI remain required; `EVID-ITEM-SPACE-CLEARING-USE-CASE-001` |
