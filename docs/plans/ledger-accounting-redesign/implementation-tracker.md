...
| Define session-ending pending-work safety boundary | ready | Exactly two comment-only target surfaces freeze environment/Principal/Account-scoped queued/applying/unresolved-rejected/unverified-attachment counts, clean/sync-first/exact-confirmed destructive dispositions, stale-summary refusal and one narrow `AccountSessionEnding` port. Executable behavior awaits exact-ready-commit CI. Auth provider/offline lease/UI copy, physical synchronization/cleanup, O-023 retention, schema/RLS/Sync, app/MCP, migration and production remain excluded; `EVID-SESSION-ENDING-POLICY-001` |
