...
| Define provider-free Client rename operation | ready | Exactly two comment-only target core/test surfaces and a complete dossier define one stable Account/Client/name/expected-revision intent, shared operation lifecycle, canonical restart and atomic rebound refusal. Behavioral implementation has not started. No Client row/current-Project projection, frozen-history rewrite, archive/delete/merge/reassignment, physical persistence, schema/RLS/Sync/provider, app/MCP or migration behavior exists; O-024/O-025 remain open; `EVID-CLIENT-RENAME-001` |
