...
| Define Account discovery and explicit selection intent | verify | Exactly two target surfaces implement environment/Principal-scoped visibility-safe Account summaries; complete/partial/stale/authoritative-empty/failure distinction; deterministic remembered-first ordering without auto-selection; exact-snapshot-bound non-authorizing selection intent; canonical restart/refusal; stable diagnostics; and one narrow `AccountQuerying` port. Exact ready commit `ae16ed4e` / run `33666363647` pass. Four focused/all 172 target tests in 40 suites, graph/contracts, both builds and local controls pass; exact implementation-commit CI remains. A-007/A-016/O-023, current authorization, physical workspace activation/switching, membership/Account writers, schema/RLS/Sync, app/MCP, migration and production remain excluded; `EVID-ACCOUNT-DISCOVERY-SELECTION-001` |
