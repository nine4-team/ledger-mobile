...
| Target Client/Project directory and browse PowerSync slice | ready | Two comment-only target leaves freeze the Account/Principal-bound adapter and provider tests before implementation. The slice reuses verified directory/projection contracts and existing Client/Project schema, grants, RLS, Sync rows and pending-create stores; it requires independent reactive completeness, membership-gated authority, pending-row visibility, missing-relationship truth, encrypted restart, cancellation, exact navigation and aggregate-safe reconciliation. Hosted Auth/Sync, archive/text/card policy, MCP list/search, migration and cutover remain excluded; `EVID-CLIENT-PROJECT-DIRECTORY-PROVIDER-001` |
