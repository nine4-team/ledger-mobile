...
| Define typed Project rename intent-to-application use case | in progress | Exactly two target leaves implement a public non-Codable Account/Project/revision/`ProjectDisplayName` intent and a generic application boundary over the verified Project rename operation. The use case constructs before one `ProjectRenaming.rename` call, validates the receipt afterward, preserves all 12 typed failures and cancellation, and bounds unknown errors. Exact READY `dc7c9779` / run `33808493496` passes. Eight focused/all 294 tests in 62 suites, local controls, repeatable generation and both builds pass; both independent implementation reviewers return GO. Immutable exact-implementation-SHA CI remains before verification promotion. It accepts no raw String and adds no normalization, read/readiness/lifecycle/no-op/UI, broader mutation, persistence, authorization, provider/schema/RLS/Sync, app/MCP, migration, hosted or production behavior. ProjectDetailView, ProjectDetailContainer and the other named source surfaces remain unadvanced; `EVID-PROJECT-RENAME-USE-CASE-001` |
