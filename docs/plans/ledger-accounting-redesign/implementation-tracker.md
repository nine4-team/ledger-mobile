...
| G3 — Implementation | in progress | Decision-independent environment foundations are active; 19 target surfaces are verified across operation, catalog, shared-list, and scoped-route slices, and 2 typed-edit surfaces are implemented pending exact-commit CI. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Define typed edit draft/submission truth | verify | Typed unchanged/set/clear fields, Account/actor/contract/subject/revision-bound drafts, validated non-decodable operation binding, stable lifecycle presentation, and stale/nonmonotonic refusal; 4 focused/39 total target tests and both target builds pass locally; exact-commit CI pending; `EVID-TYPED-EDIT-001` |
