...
| G3 — Implementation | in progress | Decision-independent foundations are active; 84 target surfaces are verified across completed provider-free slices. The count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Define Account discovery and explicit selection intent | ready | Exactly two comment-only target surfaces are ready to implement environment/Principal-scoped visibility-safe Account summaries; complete/partial/stale/authoritative-empty/failure distinction; deterministic remembered-first ordering without auto-selection; exact-snapshot-bound non-authorizing selection intent; canonical restart/refusal; and one narrow `AccountQuerying` port. All 168 existing target tests in 39 suites, graph/contracts, both builds and local controls pass; exact ready-commit CI remains. A-007/A-016/O-023, current authorization, physical workspace activation/switching, membership/Account writers, schema/RLS/Sync, app/MCP, migration and production remain excluded; `EVID-ACCOUNT-DISCOVERY-SELECTION-001` |
