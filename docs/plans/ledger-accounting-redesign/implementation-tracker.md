...
| Define provider-free Client rename operation | in progress | Exactly two target-only core/test surfaces implement one stable Account/Client/name/expected-revision intent, shared operation envelope/fingerprint/receipt/replay lifecycle, canonical restart, atomic rebound refusal and one narrow port. Four focused deterministic obligations pass locally; exact-implementation-commit CI remains planned. No Client row/current-Project projection, frozen-history rewrite, archive/delete/merge/reassignment, physical persistence, schema/RLS/Sync/provider, app/MCP or migration behavior exists; O-024/O-025 remain open; `EVID-CLIENT-RENAME-001` |
