...
| Define provider-free Project preference reads | ready | Exactly two comment-only target core/test surfaces are scoped to current-Principal Account/Project preference rows, ordered unique pinned category identities, revision, exact query/readiness evidence and authoritative not-stored versus incomplete not-available lookup. All executable tests remain planned. No authentication/authorization implementation, preference write, default-pin/category visibility resolution, budget calculation, physical persistence, schema/RLS/Sync/provider, app/MCP, migration or production behavior exists; `EVID-PROJECT-PREFERENCE-READ-001` |
