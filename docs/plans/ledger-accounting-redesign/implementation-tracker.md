...
| G3 — Implementation | in progress | Provider-free foundations are extensive, but executable migration progress is still early. One provider-backed Client create/read slice is implemented locally across spike-prefixed Postgres/RPC/RLS, encrypted PowerSync SQLite and upload handling, the isolated target app, and gated target MCP; immutable CI plus real isolated PowerSync/Auth proof remain open. The 123 previously verified `LedgerTargetCore` production/test leaves are prerequisites and must not be counted as 123 migrated features. Completion still requires every product slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md); no Firebase application adapter exists |
...
| Supabase Postgres target | in progress | Local spike-prefixed Client tables, indexes, transactional create handler, least-privilege grants, RLS and immutable operation results pass pgTAP/Data API tests; broader canonical schema, hosted rehearsal and production topology remain open |
| PowerSync target | in progress | Client slice has exact pinned SDK/SQLite dependencies, encrypted per-scope local storage, optimistic operation/read model, upload connector and draft Sync Streams locally. Real isolated Sync/Auth validation, inventory provenance, media, evolution, scale and failure phases still gate A-004 |
| Supabase Storage target | not started | Private paths, policies, resumable upload, metadata/reference reconciliation |
| Target MCP server | in progress | Gated `create_client` uses authenticated context, canonical command bytes and the same Supabase RPC/result validation as the app; server composition, remaining tools, hosted auth and deployment remain open |
...
| Access controls | in progress | Client membership/capability authorization, RLS, grants and local negative tests pass; all remaining entities plus real PowerSync stream authorization and adversarial hosted proof remain open |
