...
| Define deterministic target reference test support | ready | Two target-only comment scaffolds are mapped to validated synthetic environment/Principal/Account context, fixed clock/IDs/revisions, immutable scripted operation/readiness/failure adapters, canonical restart evidence and provider/production refusal. Current Swift/MCP helper surfaces remain target-mapped; implementation begins only after this ready gate passes; `EVID-DETERMINISTIC-TEST-SUPPORT-001` |
