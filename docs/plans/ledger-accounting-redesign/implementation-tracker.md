...
| G0.75 — Target mapping | design | 389 of 563 target-relevant surfaces are target-mapped or later; all 174 residual surfaces have explicit decision/spike/production-evidence blockers across 45 validated blocker keys; `EVID-M2-WHOLE-MANIFEST-001` plus bounded slice evidence |
...
| G3 — Implementation | in progress | Decision-independent foundations are active; 111 `LedgerTargetCore` production/test leaf surfaces are verified across completed provider-free slices, including the Space-checklist-revision-use-case leaves at exact implementation commit `99574328` / immutable run `33788214259`. This count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Define Project setup selection-to-application use case | ready | Exactly two comment-only target leaves freeze existing ProjectSetupFormSelection/current ProjectSetupFormPreparation command derivation before one ProjectSetupOperating.create call, exact receipt/local-state validation, both typed failure families, cancellation and bounded raw errors. Matching represented ready/partial/stale evidence remains admissible without a new readiness gate; preparation evidence never enters the command. Strict authority preflight and complete local READY gates pass with all 273 existing target tests in 59 suites, repeatable generation and both staging builds. Two independent reviewers found and verified scanner, optional-allocation-shape, Client-delta and discovery-count corrections and returned GO. NewProjectView, ProjectFormValidation/tests and ProjectService remain unadvanced; UI/default/category mutation/media, physical persistence, authorization, provider/schema/RLS/Sync, app/MCP, migration and production remain excluded; O-023/O-024/O-025/O-026 stay open. Exact READY-commit CI is required before implementation; `EVID-PROJECT-SETUP-USE-CASE-001` |
