...
| Define provider-free Project archive operation | ready | Exactly two comment-only target core/test surfaces and a complete dossier define one stable Project archive intent, exact expected revision, shared operation binding and explicit no-delete/no-reassignment boundary. Behavioral implementation has not started; no lifecycle row, authoritative history preservation, authorization, restore/delete/edit/reassignment, schema/RLS/Sync/provider, app/MCP or migration behavior exists; `EVID-PROJECT-ARCHIVE-001` |
