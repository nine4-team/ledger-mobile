...
| G0.75 — Target mapping | design | 327 of 491 target-relevant surfaces are target-mapped or later; all 164 residual surfaces have explicit decision/spike/production-evidence blockers; `EVID-M2-WHOLE-MANIFEST-001` plus bounded slice evidence |
...
| G3 — Implementation | in progress | Decision-independent foundations are active; 52 target surfaces are verified across the completed provider-free slices. The count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Define provider-free budget-category reference reads | verify | Exactly two target core/test surfaces implement stable Account/category identity, canonical general/itemized/fee kind, lifecycle/system/exclusion/order/revision, already-authorized local readiness, exact visible-count privacy and active/non-system/itemized eligibility. Four focused/all 108 target tests, graph/contracts, both staging builds and diff formatting pass locally; exact-commit hosted CI remains before verification. No hidden fee visibility/count, category mutation, Project allocation, physical persistence, schema/RLS/Sync/provider, app/MCP or migration behavior exists; O-026 remains open; `EVID-BUDGET-CATEGORY-REFERENCE-001` |
