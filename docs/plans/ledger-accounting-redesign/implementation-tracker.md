...
| G0.75 — Target mapping | design | 387 of 561 target-relevant surfaces are target-mapped or later; all 174 residual surfaces have explicit decision/spike/production-evidence blockers across 45 validated blocker keys; `EVID-M2-WHOLE-MANIFEST-001` plus bounded slice evidence |
...
| G3 — Implementation | in progress | Decision-independent foundations are active; 107 `LedgerTargetCore` production/test leaf surfaces are verified across completed provider-free slices, including the Project-archive-use-case leaves at commit `232acf92` / run `33758116150`. Two additional Client-archive-use-case leaves are implemented locally and pass review plus the complete local gate, but remain unverified pending exact-implementation-SHA CI. This count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Define selected-Client archive application use case | verify | Exactly two target leaves implement a transient exact Account/Client/expected-revision intent and application orchestration over the verified Client archive operation: caller operation metadata, construction before one port call, receipt validation, structured cancellation, normalized failure preservation and bounded raw errors. Exact READY commit `a4bc1daa` / run `33765054735` passed before implementation. One bounded writer changed only the two leaves; authority review caught and corrected one missing conformance-proof test; two final reviews return GO. Root line review, six focused/all 267 tests, complete controls, repeatable generation and both staging builds pass. Client reads/UI, lifecycle eligibility/projection, physical persistence, authorization, schema/RLS/Sync/provider, app/MCP, migration, hosted resources, production and cutover remain excluded. Exact implementation-SHA CI is required; `EVID-CLIENT-ARCHIVE-USE-CASE-001` |
