...
| Define Project setup selection-to-application use case | in progress | Exactly two target leaves implement existing ProjectSetupFormSelection/current ProjectSetupFormPreparation command derivation before one ProjectSetupOperating.create call, exact receipt/local-state validation, both typed failure families, cancellation and bounded raw errors. Matching represented ready/partial/stale evidence remains admissible without a new readiness gate; preparation evidence never enters the command. Exact READY commit `863f6fce` passed run `33791245215`. Root every-line review and six focused/all 279 tests pass; two independent final reviewers return GO after correcting nil-description, same-identity Client-kind and allocation-order proof gaps. NewProjectView, ProjectFormValidation/tests and ProjectService remain unadvanced; UI/default/category mutation/media, physical persistence, authorization, provider/schema/RLS/Sync, app/MCP, migration and production remain excluded; O-023/O-024/O-025/O-026 stay open. Exact implementation-SHA CI is required before verification; `EVID-PROJECT-SETUP-USE-CASE-001` |
