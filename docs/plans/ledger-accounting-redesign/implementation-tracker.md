...
| G0.75 — Target mapping | design | 291 of 455 target-relevant surfaces are target-mapped or later; all 164 residual surfaces have explicit decision/spike/production-evidence blockers; `EVID-M2-WHOLE-MANIFEST-001` plus bounded slice evidence |
...
| G3 — Implementation | in progress | Decision-independent environment foundations are active; 23 target surfaces are verified across operation, catalog, shared-list, scoped-route, typed-edit, and privacy-safe telemetry slices, and 2 release-manifest surfaces have passed their ready gate. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Map stable target responsibilities | done | 291 target mappings or later across source and target implementation surfaces; zero incomplete mapped records; 164 unresolved surfaces remain blocked rather than guessed |
...
| Define shared list/query presentation truth | done | Named profiles/sorts/filters/actions, stable cursors/ties, cached-first readiness/empty/failure reducer, structural non-enumeration, 3 focused/32 total target tests and both target builds pass locally and in exact-commit Actions run `33560578730`; `EVID-SHARED-LIST-001` |
| Define scoped route resolution/restoration | done | Registered stable routes, environment/Principal/Account restoration, opaque keys, activation-bound resolution, non-enumerating failures, explicit not-synced/retry states, and late-result refusal; 3 focused/35 total target tests and both target builds pass locally and in exact-commit Actions run `33562117852`; `EVID-SCOPED-ROUTE-001` |
| Define typed edit draft/submission truth | done | Typed unchanged/set/clear fields, Account/actor/contract/subject/revision-bound drafts, validated non-decodable operation binding, stable lifecycle presentation, and stale/nonmonotonic refusal; 4 focused/39 total target tests and both target builds pass locally and in exact-commit Actions run `33563347569`; `EVID-TYPED-EDIT-001` |
| Define privacy-safe telemetry/correlation truth | done | Closed registered signal/classes, validated environment/build scope, opaque domain-separated correlations, allowlisted bounded values, canonical offline bytes and deterministic redaction/refusal; 4 focused/43 total target tests and both target builds pass locally and in exact-commit Actions run `33565583450`; `EVID-TELEMETRY-001` |
| Define reproducible release manifest/artifact truth | implemented | Validated environment/profile/channel/build/contracts, immutable artifact and dependency-lock byte evidence, deterministic offline canonical manifest/digest, exact compatibility/tamper refusal and evidence-only non-authority; 4 focused/47 total target tests and both target builds pass locally; exact-commit CI remains; `EVID-RELEASE-MANIFEST-001` |
