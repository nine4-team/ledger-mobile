...
| Implement protected local attachment-byte durability provider | verify | Three isolated target surfaces now implement a validated environment/Principal/Account-bound SQLCipher localOnly queue and AES-GCM byte vault with atomic descriptor-relative promotion, restart/pending/corruption evidence, exact replay refusal, bounded reads and no delete/upload/success API. Independent review corrections pass: 10 focused tests with Swift warnings as errors and the actual 19-test PowerSync filter. Synchronized exact-commit CI is pending. This is a local prerequisite, not completion of hosted/physical `SPIKE-MED-001`, app composition, O-023 retention, migration or cutover; `EVID-ATTACHMENT-DURABILITY-PROVIDER-001` |
...
| S6 attachment durability | not started | `SPIKE-MED-001`; the deterministic local acceptance provider is implemented and awaiting immutable CI, but physical-device interruption/resume, low-storage/protection/soak and hosted private resolution have not started |
...
| Target Client create/read provider slice | in progress | Local spike-prefixed Postgres/RPC/RLS, encrypted PowerSync optimistic storage/upload/readback, target app and gated MCP pass; hosted Auth/Sync proof, migration correlation and broader Client lifecycle remain open; `M0-CLIENT-POWERSYNC-PROVIDER-001` |
| Target Project setup/read provider slice | in progress | Local existing/new-Client Project creation, complete category allocations, Postgres/RPC/RLS, encrypted PowerSync optimistic aggregate/upload/readback, target app, gated MCP and immutable run `33894323498` pass; hosted Auth/Sync proof remains open; `EVID-PROJECT-SETUP-PROVIDER-001` |
