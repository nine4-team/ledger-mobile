...
| Define provider-free Project details update operation | in progress | Exactly two target core/test surfaces implement one normalized optional-description replacement/clear, stable Project identity, exact expected revision, shared operation lifecycle and explicit separation from name, Client, category, media, lifecycle, child, accounting and history mutation. Exact ready commit `119e0455` / immutable run `33641244740` pass; four focused/all 152 tests, graph/contracts, both staging builds and clean artifacts pass locally. Exact implementation CI remains planned. No physical persistence, authoritative apply, schema/RLS/Sync/Auth/provider, app/MCP, migration or production behavior exists; O-023/O-024/O-025 remain open; `EVID-PROJECT-DETAILS-UPDATE-001` |
