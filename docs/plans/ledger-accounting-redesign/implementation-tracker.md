...
| Define provider-free budget-category reference reads | ready | Exactly two comment-only target core/test surfaces and a complete dossier define stable Account/category identity, canonical general/itemized/fee kind, lifecycle/system/exclusion/order/revision, already-authorized local readiness and active/non-system/itemized eligibility. Behavioral implementation has not started; no hidden fee visibility/count, category mutation, Project allocation, physical persistence, schema/RLS/Sync/provider, app/MCP or migration behavior exists; O-026 remains open; `EVID-BUDGET-CATEGORY-REFERENCE-001` |
