...
| Target local pending-work summary provider | ready | Two comment-only READY leaves define exact scoped queued/applying/unresolved-rejected/unverified-attachment counts, complete lifecycle validation, pending/missing/corrupt attachment inclusion, queue-free vault-orphan refusal, identity-bearing evidence, durable stable revision/time/fingerprint, serialized transactional journal, post-commit revalidation, monotonic same-count change detection and fail-closed storage behavior. Initial independent review returned NO-GO; the corrected package freezes exact affected schema/runtime/attachment surfaces without transferring ownership, and corrected-diff review is GO with no remaining P0-P3. Immutable READY CI remains pending. No sync, deletion, signout, Auth, switching, hosted access, Firebase, migration or cutover behavior; `EVID-PENDING-WORK-POWERSYNC-PROVIDER-001` |
