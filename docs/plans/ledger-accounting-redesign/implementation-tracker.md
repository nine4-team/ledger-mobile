...
| Isolate the Account workspace runtime by validated target environment | implemented | The target runtime now carries `ValidatedLedgerEnvironment` into bootstrap; binds bundle/environment/every persistence-relevant contract and resource/Principal/Account into opaque contained database and composite Keychain identity; refuses invalid namespace evidence before side effects; reopens exact encrypted queued state; and exposes only non-destructive close while fixtures clean up explicitly. The staging shell retains validated evidence. Exact READY `428f5225` / run `33927257913` and exact implementation `3bf8e9ed` / run `33928708863` pass all three immutable jobs. Two independent executable review rounds strengthened per-field and Keychain assertions; final corrected-diff review is GO with no remaining P0-P3. Four focused/all 372 tests in 71 suites and both builds pass. Pending-work aggregation, activation/switching and physical session ending remain separate; no Auth, hosted Sync, migration, Firebase or production behavior; `EVID-ACCOUNT-WORKSPACE-RUNTIME-ISOLATION-001` |
| Target local pending-work summary provider | implemented | The internal target-local provider derives exact scoped queued/applying/unresolved-rejected/unverified-attachment counts; validates every lifecycle/identity/scope row; includes pending/missing/corrupt attachment evidence; refuses queue-free or unavailable vault evidence; preserves restart-stable revision/time/fingerprint; and uses a non-reentrant whole-summary permit, transactional journal, cross-store revalidation and final journal reread. Exact READY `2072af47` / run `33930443887` and exact implementation `3ac05bd0` / run `33932915626` passed all three immutable jobs. Executable review drove corrections for the actor ABA race, extreme-time trap, real store/restart/fault coverage and false runtime-composition claim; final independent corrected-diff review returned GO with no P0-P3. Sixteen provider, 13 attachment and all 391 target tests in 72 suites plus both staging builds and isolated Supabase checks pass. The provider intentionally remains uncomposed; a later session-ending slice must own one instance and the attachment database/vault/key lifecycle. No sync, deletion, signout, Auth, switching, hosted access, Firebase, migration or cutover behavior; `EVID-PENDING-WORK-POWERSYNC-PROVIDER-001` |
