...
| Define provider-free Client archive operation | in progress | Exactly two target core/test surfaces implement one stable Client archive intent, reuse the typed expected Client revision, bind the exact same-subject precondition to the shared operation lifecycle and forbid Project cascade, hard delete, merge, rename, restore, history rewrite or reassignment. Exact ready commit `9f3a03dd` / immutable run `33638235220` pass; four focused/all 148 tests, graph/contracts, both staging builds and clean artifacts pass locally. Exact implementation CI remains planned. No physical persistence, authoritative apply, schema/RLS/Sync/Auth/provider, app/MCP, migration or production behavior exists; O-025 remains open; `EVID-CLIENT-ARCHIVE-001` |
