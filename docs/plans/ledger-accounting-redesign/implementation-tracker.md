...
| Define provider-free Project category configuration reads | ready | Exactly two comment-only target core/test surfaces and a complete dossier define exact Account/Project/category scope, no-relationship versus enabled-null/zero/positive allocation, incomplete relationship evidence, authorized local readiness and one configuration revision. Behavioral implementation has not started; no budget arithmetic, financial authorization, category/Project mutation, physical persistence, schema/RLS/Sync/provider, app/MCP or migration behavior exists; O-026 remains open; `EVID-PROJECT-CATEGORY-CONFIGURATION-001` |
