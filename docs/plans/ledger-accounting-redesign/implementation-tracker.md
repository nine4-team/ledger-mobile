...
| Isolate the Account workspace runtime by validated target environment | ready | Two new comment-only leaves plus the unchanged existing Account runtime/staging/shared-directory-test surfaces freeze the prerequisite hardening: carry `ValidatedLedgerEnvironment` into bootstrap; bind bundle/environment/persistence-relevant manifest/Principal/Account into opaque database and key namespaces; validate before side effects; reopen exact encrypted state; and remove ordinary destructive close while test fixtures clean up explicitly. A changed persistence binding selects a separate namespace; no caller-authored or separately persisted comparison value is introduced. Pending-work aggregation and physical session ending follow separately. Corrected-diff independent READY review is GO with no remaining P0-P3; immutable READY CI remains pending. No Auth, hosted Sync, migration, Firebase or production behavior; `EVID-ACCOUNT-WORKSPACE-RUNTIME-ISOLATION-001` |
