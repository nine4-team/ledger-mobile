...
| G0.75 — Target mapping | design | 393 of 567 target-relevant surfaces are target-mapped or later; all 174 residual surfaces have explicit decision/spike/production-evidence blockers across 45 validated blocker keys; `EVID-M2-WHOLE-MANIFEST-001` plus bounded slice evidence |
...
| G3 — Implementation | in progress | Decision-independent foundations are active; 113 `LedgerTargetCore` production/test leaf surfaces are verified across completed provider-free slices, including the Project-setup-use-case leaves at exact implementation commit `d624c2e0` / immutable run `33793469069`. This count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Define Project description form-to-application use case | ready | Exactly two comment-only target leaves freeze a transient exact Account/Project/revision/raw-description input with a public four-argument initializer, reuse of canonical `ProjectDescriptionReplacement` plus verified draft/command, construction before one `ProjectDetailsUpdating.updateDetails` call, receipt validation, every typed failure, cancellation and bounded unknown errors. Ten tests are planned, including non-`@testable` consumer compilation. The complete local READY gate passes all 279 target tests in 60 suites, conversion/target controls, repeatable generation and both builds. Independent review caught and corrected public-access, lifecycle-status and stale-evidence defects; both corrected-diff reviewers return GO. Immutable exact-SHA CI is the sole remaining prerequisite before executable implementation. EditProjectModal, ProjectService/Protocol, Project model and update_project MCP tool retain prior statuses; all read/readiness/lifecycle/no-op/UI, broader mutation, persistence, authorization, provider/schema/RLS/Sync, app/MCP, migration, hosted and production behavior remains excluded; `EVID-PROJECT-DETAILS-UPDATE-USE-CASE-001` |
