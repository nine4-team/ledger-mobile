...
| G3 — Implementation | in progress | Decision-independent environment foundations are active; 21 target surfaces are verified across operation, catalog, shared-list, scoped-route, and typed-edit slices, and 2 privacy-safe telemetry surfaces are implemented pending exact-commit CI. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Define privacy-safe telemetry/correlation truth | verify | Closed registered signal/classes, validated environment/build scope, opaque domain-separated correlations, allowlisted bounded values, canonical offline bytes and deterministic redaction/refusal; 4 focused/43 total target tests and both target builds pass locally; exact-commit CI pending; `EVID-TELEMETRY-001` |
