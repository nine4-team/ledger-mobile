...
| G3 — Implementation | in progress | Decision-independent foundations are active; 117 `LedgerTargetCore` production/test leaf surfaces are verified across completed provider-free slices. Two additional Client-rename-use-case leaves are implemented locally after exact READY `9cc34f4c` / run `33819792298`, but are not counted as verified until their exact implementation commit passes immutable CI. This count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Define typed Client rename intent-to-application use case | in progress | Exactly two target leaves implement a public non-Codable Account/Client/revision/`ClientDisplayName` intent and a generic application boundary over the verified Client rename operation. The use case constructs before one `ClientRenaming.rename` call, validates the receipt afterward, preserves all 12 typed failures and cancellation, and bounds unknown errors. Exact READY `9cc34f4c` / run `33819792298`, eight focused/all 302 tests in 63 suites, complete local controls, repeatable generation, both builds and two independent implementation reviews pass. One review caught and corrected missing explicit UTF-8 byte-equality evidence for the large Unicode payload. The two leaves remain implemented rather than verified until their exact implementation commit passes immutable CI. It accepts no raw String and adds no normalization, read/readiness/lifecycle/no-op/UI, archive/delete/merge, Project reassignment/projection, frozen-history rewrite, persistence, authorization, provider/schema/RLS/Sync, app/MCP, migration, hosted or production behavior; `EVID-CLIENT-RENAME-USE-CASE-001` |
