...
| G0.75 — Target mapping | design | 397 of 571 target-relevant surfaces are target-mapped or later; all 174 residual surfaces have explicit decision/spike/production-evidence blockers across 45 validated blocker keys; `EVID-M2-WHOLE-MANIFEST-001` plus bounded slice evidence |
...
| G3 — Implementation | in progress | Decision-independent foundations are active; 117 `LedgerTargetCore` production/test leaf surfaces are verified across completed provider-free slices, including the Project-rename-use-case leaves at exact implementation commit `9c8e27cf` / immutable run `33816585123`. Two additional comment-only Client-rename-use-case leaves are `target_mapped` under a `ready` dossier and are not counted as verified. This count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Map stable target responsibilities | done | 385 target mappings or later across source and target implementation surfaces; zero incomplete mapped records; 174 unresolved surfaces remain tied to 45 explicit blockers rather than guessed |
| Generate exact residual decision queue | done | 174 residual surfaces grouped under 45 validated blockers; `npm run conversion:residuals:check` |
| Draft product decision packets | done | Eighteen proposed packets cover all 37 product blockers, including O-039 Project-note text validation; O-021 is UI-only; none is approved by documentation alone |
...
| Define typed Client rename intent-to-application use case | ready | Exactly two comment-only target leaves freeze a public non-Codable Account/Client/revision/`ClientDisplayName` intent and a generic application boundary over the verified Client rename operation. Future implementation must construct before one `ClientRenaming.rename` call, validate the receipt afterward, preserve all 12 typed failures and cancellation, and bound unknown errors. The ten planned obligations require ordinary import without `@testable`, literal non-mirrored encoded-leaf ownership and exact allowlists. It accepts no raw String and adds no normalization, read/readiness/lifecycle/no-op/UI, archive/delete/merge, Project reassignment/projection, frozen-history rewrite, persistence, authorization, provider/schema/RLS/Sync, app/MCP, migration, hosted or production behavior. Exact READY commit and immutable CI remain required before implementation; `EVID-CLIENT-RENAME-USE-CASE-001` |
