...
| Define provider-free Space-template reference reads | ready | Exactly two target-only comment scaffolds have a complete ready dossier for stable Account/template/checklist/item identity, text, lifecycle/revision/order, structure with no checked state, explicit ready/partial/stale/authoritative-empty local evidence and one narrow query port. Exact ready-commit CI is required before executable implementation. No O-026 mutation authority, apply/save, Space creation, physical persistence, schema/RLS/Sync/provider, app/MCP, migration or production behavior exists; `EVID-SPACE-TEMPLATE-REFERENCE-001` |
