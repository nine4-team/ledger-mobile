...
| G3 — Implementation | in progress | Decision-independent foundations are active; 44 target surfaces are verified across the completed provider-free slices, while two attachment-capture receipt scaffolds are ready. The count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Define attachment capture/local-durability receipt boundary | ready | Two target-only core/test scaffolds map stable Attachment ID and exact environment/Principal/Account/parent scope, non-Codable raw capture bytes, adapter-reported size/SHA-256/opaque-local-object evidence, canonical path-free receipt, narrow capture-store port and fail-closed tests. Physical encrypted persistence, upload/display/Storage, O-023 retention, providers, app/MCP and migration remain unimplemented; `EVID-ATTACHMENT-CAPTURE-001` |
