...
| G3 — Implementation | in progress | Decision-independent environment foundations are active; 15 target surfaces are verified across operation/catalog slices and 2 shared-list surfaces are implemented pending exact-commit CI. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Define shared list/query presentation truth | verify | Named profiles/sorts/filters/actions, stable cursors/ties, cached-first readiness/empty/failure reducer, structural non-enumeration, 3 focused tests and both target builds pass locally; exact-commit CI pending; `EVID-SHARED-LIST-001` |
