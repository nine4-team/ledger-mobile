...
| G3 — Implementation | in progress | Decision-independent foundations are active; 109 `LedgerTargetCore` production/test leaf surfaces are verified across completed provider-free slices, including the Client-archive-use-case leaves at exact implementation commit `a5ba0ae3` / immutable run `33767120461`. Two additional Space-checklist-revision-use-case leaves are comment-only READY candidates and are not implementation-advanced. This count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Define Space checklist revision edit-to-application use case | ready | Exactly two comment-only target leaves freeze existing restart-safe SpaceChecklistEditingDraft plus current SpaceCoreDetailsUpdate command derivation, one post-derivation `SpaceChecklistRevising` call, receipt validation, typed presentation/revision failure preservation, Swift cancellation and bounded raw errors. Canonical authority covers complete ordered hierarchy replacement, stable nested IDs, normalization, checked state, empty/zero-item validity and atomic revision conflict. EditChecklistModal remains `target_mapped`; UI admission/controls, physical persistence, authorization, provider/schema/RLS/Sync, app/MCP, migration and production remain excluded. All 267 existing target tests, complete controls, repeatable generation and both staging builds pass; actual-diff reviews and exact-ready-SHA CI remain required; `EVID-SPACE-CHECKLIST-REVISION-USE-CASE-001` |
