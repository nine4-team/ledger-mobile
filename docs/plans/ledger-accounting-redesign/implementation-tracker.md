...
| Define provider-free Client archive operation | ready | Exactly two comment-only target core/test surfaces freeze one stable Client archive intent, reuse the typed expected Client revision, bind the exact same-subject precondition to the shared operation lifecycle and forbid Project cascade, hard delete, merge, rename, restore, history rewrite or reassignment. Implementation awaits immutable ready-commit CI. No physical persistence, authoritative apply, schema/RLS/Sync/Auth/provider, app/MCP, migration or production behavior exists; O-025 remains open; `EVID-CLIENT-ARCHIVE-001` |
