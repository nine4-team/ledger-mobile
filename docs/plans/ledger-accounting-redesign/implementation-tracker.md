...
| Define Project archive intent-to-application use case | ready | Exactly two new comment-only target leaves freeze a non-Codable exact Account/Project/expected-revision intent and one application use case that composes the existing verified revision/command/receipt semantics, calls `ProjectArchiving` once after construction, preserves cancellation/normalized failure and bounds raw errors. Source `ProjectDetailView` remains characterized under O-024. The source `archive_project` Boolean remains completely target-mapped to the existing `ArchiveProjectCommand` and separate future `RestoreProjectCommand`, but this slice advances only archive; no read/readiness/lifecycle eligibility, restore implementation, MCP wiring, delete, physical history proof, UI, provider/schema/RLS/Sync, migration or production behavior advances. Review caught and drove correction of six MCP-accounting, revision-authority, reconciliation and continuity defects; the complete corrected local ready gate passes. Baseline promotion commit `d74efb44` passed run `33751486795`; final corrected-diff re-review and exact-ready-SHA CI remain required; `EVID-PROJECT-ARCHIVE-USE-CASE-001` |
