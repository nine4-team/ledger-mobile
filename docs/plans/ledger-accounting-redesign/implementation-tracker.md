...
| Define provider-free Item-to-Space assignment operation | verify | Exactly two target core/test surfaces implement one nonempty canonical same-scope `AssignItemsToSpace` intent with stable destination Space/Item identities, exact Space/Item revisions, deterministic active/revision/scope preconditions, atomic refusal, stable diagnostics, shared operation lifecycle and one narrow port. Exact ready commit `30f4ab88` and immutable run `33670383410` pass; four focused/all 176 target tests in 41 suites, graph/contracts, both builds, repeatable project generation and local controls pass, while exact implementation-commit CI remains. O-037 stays open because archive is absent; clear assignment, Item scope/accounting mutation, physical persistence, schema/RLS/Sync/provider behavior, app/MCP, migration and production remain excluded; `EVID-ITEM-SPACE-ASSIGNMENT-001` |
