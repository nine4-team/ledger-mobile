...
| Enforce complete target query-port method inventory | ready | Two comment-only control leaves freeze a dependency-free fail-closed scanner and adversarial suite for every direct method requirement in public exact-suffix `Querying` protocols. Baseline is 16 verified owners / 16 protocols / 18 methods, all current `watch*` observations, with stable TQUERY identity and separate signature drift. Implementation will add deterministic JSON/Markdown, package generate/check/test commands, an Ubuntu conversion-control hook and a dependency that prevents the macOS target job from starting after inventory failure; later slices own every-TQUERY logical access/index review and reconciliation to all 386 source `QUERY-*` occurrences. No product/query semantics, physical index, provider, hosted or production behavior advances; `EVID-TARGET-QUERY-PORT-INVENTORY-001` |
