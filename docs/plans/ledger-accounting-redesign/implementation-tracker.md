...
| Define provider-free Item Space-assignment clear operation | ready | Exactly two comment-only target surfaces freeze one nonempty canonical same-scope `ClearItemSpaceAssignments` intent with stable Item/current-Space identities, exact Item revisions, deterministic Space-scope plus Item-revision/scope/current-placement preconditions, atomic refusal, marker-relationship closure without media deletion, shared operation lifecycle and one narrow port. All 176 existing target tests in 41 suites, graph/contracts, both builds, repeatable project generation and local controls pass; exact ready-commit CI remains. O-037 stays open because archive is absent; O-023 stays open because references/bytes are not deleted; destination assignment, Item scope/accounting mutation, physical persistence, schema/RLS/Sync/provider behavior, app/MCP, migration and production remain excluded; `EVID-ITEM-SPACE-CLEARING-001` |
