...
| Define exact Money and stable domain identity | ready | Two target-only core/test scaffolds map signed Int64 minor-unit Money plus explicit validated currency, distinct stable entity IDs, checked same-currency arithmetic, deterministic restart encoding and malformed/cross-currency/overflow refusal. Product signs/bounds, currency policy, formatting, tax/rounding, schema, providers, app/MCP and migration remain unadvanced; `EVID-DOMAIN-PRIMITIVES-001` |
