...
| Define provider-free Project setup operation | ready | Exactly two comment-only target core/test surfaces and a complete dossier define the bounded existing/new Client selection, stable Project identity/name/description, complete canonical category set, absent/null/zero allocation, shared operation and separate-media contract. Behavioral implementation has not started; no rows, authorization, category-definition mutation, Project edit/lifecycle, media, schema/RLS/Sync/provider, app/MCP or migration behavior exists; `EVID-PROJECT-SETUP-001` |
