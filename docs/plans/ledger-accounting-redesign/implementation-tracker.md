...
| Define provider-free Project rename operation | in progress | Exactly two target-only core/test surfaces implement one stable Account/Project/name/expected-revision intent, shared operation envelope/fingerprint/receipt/replay lifecycle, canonical restart, atomic rebound refusal and one narrow port. Four focused deterministic obligations pass locally; exact-implementation-commit CI remains planned. No Project row/downstream projection, Client/description/category/media/history/lifecycle change, physical persistence, schema/RLS/Sync/provider, app/MCP or migration behavior exists; O-024/O-025 remain open; `EVID-PROJECT-RENAME-001` |
