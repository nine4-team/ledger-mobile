...
| Target Account discovery and explicit-selection PowerSync slice | ready | Four comment-only target leaves freeze the Principal/environment-scoped local Account query, encrypted bootstrap runtime, false-empty/readiness/freshness/failure/restart/cancellation tests and a build-only explicit-selection staging component. Existing staging-root runtime wiring, server schema, grants, RLS, Data API and the current broad illustrative Sync YAML are explicitly excluded. No workspace activation, Auth/offline lease, pending-work disposal, MCP state, migration, hosted or production behavior exists. Implementation waits for corrected independent READY review and immutable CI; `EVID-ACCOUNT-DISCOVERY-PROVIDER-001` |
