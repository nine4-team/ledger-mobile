...
| G3 — Implementation | in progress | Decision-independent foundations are active; 109 `LedgerTargetCore` production/test leaf surfaces are verified across completed provider-free slices, including the Client-archive-use-case leaves at exact implementation commit `a5ba0ae3` / immutable run `33767120461`. This count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
