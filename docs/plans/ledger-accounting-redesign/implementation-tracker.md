...
| Freeze complete target-query logical-authority crosswalk | implemented | Reviewed all 18 generated TQUERY methods into one closed JSON registry bound to the inventory digest and exact signature hashes. A dependency-free generator/checker and 23-test adversarial suite now fully re-derive upstream inventory schemas/relations/totals/identities, enforce review-class consistency, canonicalize nested output, reject root/symlink escapes, use exclusive no-follow atomic generation, validate exact CLI arity, derive collision-checked TACCESS identities and separate mapping hashes, and emit/check one deterministic JSON artifact. Root generate/check/test commands and the Ubuntu conversion-control gate are wired before the dependent macOS target job. The amended READY exclusions let the tests retain contiguous `firestoreQuery`/`firestoreIndex` negative cases while both conversion surface discovery and the 386-occurrence current-query inventory remain unchanged. Human review confirms all 18 generated rows and explicit uncertainty; machine validation performs no free-text or Markdown semantic/status inference. Postgres and local physical planes remain globally deferred under proposed A-003/A-004 with no TINDEX IDs, SQL, RLS, Sync assignment or provider/runtime claim. Only the two control/test surfaces advance to `implemented`; discoverer/query-extractor/package/workflow retain their existing status and ownership, all query owners remain unadvanced, and immutable implementation CI remains pending. Reconciliation of all 386 source `QUERY-*` occurrences is a separate later slice; `EVID-TARGET-QUERY-LOGICAL-AUTHORITY-001` |
