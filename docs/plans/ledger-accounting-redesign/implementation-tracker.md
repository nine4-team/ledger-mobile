...
| G3 — Implementation | in progress | Decision-independent foundations are active; 80 target surfaces are verified across the completed provider-free slices, with two additional Space-checklist surfaces implemented locally and awaiting exact-commit CI. The count deliberately excludes broader source app/MCP/test-helper surfaces until their target integrations pass. Completion still requires every target slice to pass the [Vertical Slice Implementation Method](conversion/vertical-slice-implementation-method.md), including Postgres/grants/RLS/PowerSync where applicable; no Firebase application adapter exists |
...
| Define provider-free Space checklist revision operation | verify | Exactly two target core/test surfaces implement complete canonical ordered checklist replacement, stable nested identities, checked-state progress, exact same-Space expected revision, canonical restart/refusal, shared operation lifecycle and one narrow port. Four focused/all 164 target tests, graph/contracts and both builds pass locally; exact-implementation-commit CI remains. Scope/details/template/media/Item/review/Space-completion/lifecycle/accounting mutation, authorization, physical persistence, schema/RLS/Sync/provider behavior, app/MCP, migration and production remain excluded; O-023/O-026/O-032/O-037 stay open; `EVID-SPACE-CHECKLIST-REVISION-001` |
