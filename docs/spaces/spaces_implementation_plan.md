Our transaction details screen is not good. Look at how we changed the item details screen and update the transaction details screen to align with the changes we made for it. If you need to, you can reference our legacy web app, which we're migrating from, because it will tell you a lot of what has to be present and how stuff works.# Spaces Feature Implementation Plan
