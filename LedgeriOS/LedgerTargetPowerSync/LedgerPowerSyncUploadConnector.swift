...
public struct SpaceChecklistRevisionUploadRequest: Equatable, Sendable {
    public let operationId: String
    public let accountId: String
    public let actorPrincipalId: String
    public let contractVersion: String
...
public struct SpaceChecklistRevisionServerResult: Codable, Equatable, Sendable {
    public let operationId: String
    public let accountId: String
    public let actorPrincipalId: String
    public let commandType: String
...
public protocol SpaceChecklistRevisionCommandApplying: Sendable {
    func apply(
...
public struct LedgerPowerSyncCommandAppliers: Sendable {
    ...
    var transactionReceiptLinesEdit: (any EditTransactionReceiptLinesApplying)?
    var uninvoicedReturn: (any ReturnUninvoicedItemsCommandApplying)?
    var sourceReturn: (any InventorySourceReturnCommandApplying)?
    ...
    public init(clientCreation: any ClientCreationCommandApplying,
                ...
                transactionReceiptLinesEdit: (any EditTransactionReceiptLinesApplying)? = nil,
                uninvoicedReturn: (any ReturnUninvoicedItemsCommandApplying)? = nil,
                sourceReturn: (any InventorySourceReturnCommandApplying)? = nil,
        ...
        self.transactionReceiptLinesEdit = transactionReceiptLinesEdit
        self.uninvoicedReturn = uninvoicedReturn
        self.sourceReturn = sourceReturn
...
final class LedgerPowerSyncUploadConnector: PowerSyncBackendConnectorProtocol, @unchecked Sendable {
    ...
    private let transactionReceiptLinesEditApplier: (any EditTransactionReceiptLinesApplying)?
    private let uninvoicedReturnApplier: (any ReturnUninvoicedItemsCommandApplying)?
    private let sourceReturnApplier: (any InventorySourceReturnCommandApplying)?
    ...
    init(
        ...
        transactionReceiptLinesEditApplier: (any EditTransactionReceiptLinesApplying)? = nil,
        uninvoicedReturnApplier: (any ReturnUninvoicedItemsCommandApplying)? = nil,
        sourceReturnApplier: (any InventorySourceReturnCommandApplying)? = nil,
        ...
        self.transactionReceiptLinesEditApplier = transactionReceiptLinesEditApplier
        self.uninvoicedReturnApplier = uninvoicedReturnApplier
        self.sourceReturnApplier = sourceReturnApplier
    ...
    public func uploadData(database: any PowerSyncDatabaseProtocol) async throws {
        ...
        case LedgerPowerSyncTable.itemDetailsEditCommands:
            guard let itemDetailsEditApplier else {
                throw LedgerPowerSyncUploadFailure.unsupportedCommandTable(entry.table)
            }
            try await ItemDetailsEditUpload.apply(entry, database: database,
                accessFence: accessFence, applier: itemDetailsEditApplier)
        case LedgerPowerSyncTable.transactionReceiptLinesEditCommands:
            guard let transactionReceiptLinesEditApplier else {
                throw LedgerPowerSyncUploadFailure.unsupportedCommandTable(entry.table)
            }
            try await TransactionDetailsEditUpload.applyReceiptLines(entry, database: database,
                accessFence: accessFence, applier: transactionReceiptLinesEditApplier)
        ...
        case LedgerPowerSyncTable.itemPriceEditCommands:
            ...
            try await ItemPriceEditUpload.apply(entry, database: database,
                accessFence: accessFence, applier: itemPriceEditApplier)
        case LedgerPowerSyncTable.uninvoicedReturnCommands:
            guard let uninvoicedReturnApplier else {
                throw LedgerPowerSyncUploadFailure.unsupportedCommandTable(entry.table)
            }
        ...
        case LedgerPowerSyncTable.sourceReturnCommands:
            guard let sourceReturnApplier else {
                throw LedgerPowerSyncUploadFailure.unsupportedCommandTable(entry.table)
            }
            try await InventorySourceReturnUpload.apply(entry, database: database,
                accessFence: accessFence, applier: sourceReturnApplier)
        ...
        case LedgerPowerSyncTable.spaceChecklistRevisionCommands:
            guard let spaceChecklistRevisionApplier else {
                throw LedgerPowerSyncUploadFailure.unsupportedCommandTable(entry.table)
            }
            let request = try Self.spaceChecklistRevisionRequest(from: transaction.crud)
            ...
            try await recordSpaceChecklistRevisionResult(
                result,
                request: request,
                database: database
            )
    ...
    private func markSpaceChecklistRevisionApplying(
        _ request: SpaceChecklistRevisionUploadRequest,
        ...
        try await database.writeTransaction { local in
            ...
            guard let operation,
                  ...
                  operation.subjectId == request.spaceId,
                  operation.commandType == "revise_space_checklists",
            ...
            let overlays = try local.getAll(
                ...
                """,
                parameters: [request.operationId]
            ) { try SpaceChecklistRevisionUploadOverlay(cursor: $0) }
    ...
    private func resetSpaceChecklistRevisionAfterTransientFailure(
        _ request: SpaceChecklistRevisionUploadRequest,
        database: any PowerSyncDatabaseProtocol
    ) async throws {
        let observedAt = try Self.milliseconds(now())
        try await database.writeTransaction { local in
            let times = try local.getOptional(
                ...
                (
                    ...
                    try $0.getInt64(name: "updated_at_ms")
                )
            }
    ...
    private func recordSpaceChecklistRevisionResult(
        _ result: SpaceChecklistRevisionServerResult,
        request: SpaceChecklistRevisionUploadRequest,
        database: any PowerSyncDatabaseProtocol
    ) async throws {
        let observedAt = try Self.milliseconds(now())
        try await database.writeTransaction { local in
    ...
    static func spaceChecklistRevisionRequest(
        from entries: [CrudEntry]
    ) throws -> SpaceChecklistRevisionUploadRequest {
        guard entries.count == 1,
              let commandEntry = entries.first,
              commandEntry.table == LedgerPowerSyncTable.spaceChecklistRevisionCommands,
              commandEntry.op == .put,
              let commandData = commandEntry.opData else {
            throw LedgerPowerSyncUploadFailure.invalidTransactionShape
        }
        ...
        let request = SpaceChecklistRevisionUploadRequest(
            ...
            spaceId: try required("space_id"),
            expectedRevision: try required("expected_revision"),
            collectionJSON: try required("collection_json"),
            fingerprint: try required("fingerprint"),
            envelopeJSON: try required("envelope_json")
        )
        guard let revision = UInt64(request.expectedRevision),
              ...
              let collectionData = request.collectionJSON.data(using: .utf8),
              ...
              envelope.actorPrincipalId.rawValue == request.actorPrincipalId,
              envelope.contractVersion.rawValue == request.contractVersion,
              envelope.payload.spaceId.rawValue == request.spaceId,
              envelope.payload.collection == collection,
              envelope.clientCreatedAt == Date(
              ...
              envelope.preconditions == [
                .expectedRevision(
                    subject: LedgerEntityReference(
                        kind: .space,
                        id: try EntityID(validating: request.spaceId)
                    ),
                    revision: revision
                )
              ],
              let fingerprint = try? OperationFingerprint(
                  validating: request.fingerprint
              ),
              (try? OperationFingerprint.make(for: envelope)) == fingerprint else {
            throw LedgerPowerSyncUploadFailure.invalidArchiveCommand
        }
    ...
    static func isValidSpaceChecklistRevisionResult(
        _ result: SpaceChecklistRevisionServerResult,
        request: SpaceChecklistRevisionUploadRequest
    ) -> Bool {
        guard result.operationId == request.operationId,
              ...
              result.commandType == "revise_space_checklists",
              result.contractVersion == request.contractVersion,
              result.commandFingerprint == request.fingerprint,
              result.envelopeSHA256 == request.fingerprint,
              result.requestSHA256 == spaceChecklistRevisionRequestSHA256(request),
              result.subjectId == request.spaceId,
              result.clientCreatedAtMilliseconds == request.clientCreatedAtMilliseconds,
              result.completedAtMilliseconds >= result.serverReceivedAtMilliseconds
        else {
            return false
        }
        switch result.phase {
        case "applied":
            return result.resultCode == "space_checklists_revised"
                && result.errorCode == nil
        case "rejected":
            return result.resultCode == nil
                && result.errorCode.map(
    ...
    static func spaceChecklistRevisionRequestSHA256(
        _ request: SpaceChecklistRevisionUploadRequest
    ) -> String {
        let fields = [
            request.operationId,
            ...
            request.collectionJSON,
            request.fingerprint,
            request.envelopeJSON
        ]
        var material = Data("space-checklist-revision-request-v1|".utf8)
        for field in fields {
...
private struct SpaceChecklistRevisionUploadOverlay {
    let accountId: String
    let actorPrincipalId: String
    let spaceId: String
    let operationId: String
    ...
    func matches(
        ...
        return accountId == request.accountId
            ...
            && expectedRevision == request.expectedRevision
            && projectedRevision == revision + 1
            && collectionJSON == request.collectionJSON
            && acceptedAtMilliseconds == expectedAcceptedAt
    }
}
