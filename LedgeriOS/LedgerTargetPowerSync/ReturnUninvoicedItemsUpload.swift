...
enum ReturnUninvoicedItemsUpload {
    static func apply(_ entry: CrudEntry, database: any PowerSyncDatabaseProtocol,
                      accessFence: LedgerWorkspaceAccessFence,
                      applier: any ReturnUninvoicedItemsCommandApplying) async throws {
        guard entry.op == .put, entry.table == LedgerPowerSyncTable.uninvoicedReturnCommands,
        ...
        let command = try OperationContractCodec.decode(ReturnUninvoicedItemsCommand.self,
            from: Data("{\"envelope\":\(json)}".utf8))
        let e = command.envelope, request = try ReturnUninvoicedItemsUploadRequest(command)
        guard entry.id == e.operationId.rawValue, data["account_id"] == e.accountId.rawValue,
              ...
              AccountBoundOperationIdentity.isValid(e.operationId, family: .uninvoicedReturn, accountId: e.accountId) else {
            throw LocalOperationIdentityGuardFailure.malformedEvidence
        }
    ...
    private static func requireOwner(_ local: any Transaction, command: ReturnUninvoicedItemsCommand,
