...
enum ReturnPaidItemsUpload {
    static func apply(_ entry: CrudEntry, database: any PowerSyncDatabaseProtocol,
                      accessFence: LedgerWorkspaceAccessFence,
                      applier: any ReturnPaidItemsCommandApplying) async throws {
        guard entry.op == .put, entry.table == LedgerPowerSyncTable.paidReturnCommands,
        ...
        let command = try OperationContractCodec.decode(ReturnPaidItemsCommand.self,
            from: Data("{\"envelope\":\(json)}".utf8))
        let e = command.envelope, request = try ReturnPaidItemsUploadRequest(command)
        guard entry.id == e.operationId.rawValue, data["account_id"] == e.accountId.rawValue,
              ...
              data["project_id"] == e.payload.projectId.rawValue,
              ...
              AccountBoundOperationIdentity.isValid(e.operationId, family: .paidReturn, accountId: e.accountId) else {
            throw LocalOperationIdentityGuardFailure.malformedEvidence
        }
    ...
    private static func requireOwner(_ local: any Transaction, command: ReturnPaidItemsCommand,
                                     fingerprint: String, fence: LedgerWorkspaceAccessFence) throws {
        try requireAccess(fence)
        try ProjectInvoicingItemLocalReader.requireAccess(transaction: local, accountId: command.envelope.accountId,
            principalId: command.envelope.actorPrincipalId, projectId: command.envelope.payload.projectId)
