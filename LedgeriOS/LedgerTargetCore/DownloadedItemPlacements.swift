...
public struct DownloadedItemGroup: Equatable, Sendable, Identifiable {
    public typealias ID = ItemGrouping.Key
    public let id: ID
...
public struct DownloadedItemPlacements: Equatable, Sendable {
    ...
    public func groups(for itemIds: [ItemID], in spaceId: SpaceID? = nil) -> [DownloadedItemGroup] {
        ItemGrouping.groups(in: rows(in: spaceId), selectedIDs: itemIds, id: \.itemId,
            name: \.displayName, sku: \.sku, source: \.source)
            .map { .init(id: $0.id, representative: $0.representative, rows: $0.rows) }
    }
...
public struct PhysicalItemPlacementHistoryInterval: Equatable, Sendable {
    ...
    public enum StartEvidence: String, Sendable {
        case recordedMove = "recorded_move"
        case importObservation = "import_observation"
        case unknown
    }
    public let startEvidence: StartEvidence
    public var startDescription: String {
        switch startEvidence {
        case .recordedMove: "From: \(startedAt)"
        case .importObservation: "Source location imported: \(startedAt) — move date unknown"
        case .unknown: "Recorded: \(startedAt) — move date unknown"
        }
    }
    ...
    public init(placementId: EntityID, scope: ItemPlacementScope, spaceId: SpaceID?,
                ...
                startedAt: String, endedAt: String?, startEvidence: StartEvidence = .recordedMove) {
        self.placementId = placementId; self.scope = scope; self.spaceId = spaceId
...
public struct DownloadedItemReturnLink: Equatable, Sendable, Identifiable {
    ...
    public init(id: EntityID, chargeId: BillableItemOccurrenceID, projectId: ProjectID,
                projectPlacementId: EntityID, inventoryPlacementId: EntityID) {
        self.id = id; self.chargeId = chargeId; self.projectId = projectId
        self.projectPlacementId = projectPlacementId; self.inventoryPlacementId = inventoryPlacementId
    }
}
...
public struct DownloadedItemPlacementHistory: Equatable, Sendable {
    ...
    public init(accountId: AccountID, itemId: ItemID, description: String,
        ...
        self.pendingSale = pendingSale
        guard Set(returnLinks.map(\.id)).count == returnLinks.count,
              Set(returnLinks.map(\.chargeId)).count == returnLinks.count,
              returnLinks.allSatisfy({ link in
                  guard let from = intervals.first(where: { $0.placementId == link.projectPlacementId }),
                        let to = intervals.first(where: { $0.placementId == link.inventoryPlacementId }) else { return false }
                  return from.scope == .project(link.projectId) && to.scope == .businessInventory
                    && from.endedAt == to.startedAt && from.placementId != to.placementId
              }) else { throw DownloadedItemPlacementsFailure.scopeMismatch }
        self.returnLinks = returnLinks
    }
}
