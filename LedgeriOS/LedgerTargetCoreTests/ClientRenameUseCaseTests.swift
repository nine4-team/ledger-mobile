// READY CONTRACT — CLIENT RENAME USE CASE TESTS
//
...
//    the exact public four-argument ClientRenameIntent initializer. The intent
//    is Equatable and Sendable, runtime non-Codable, and stores exactly Account,
//    Client, expected revision and already-validated ClientDisplayName.
// 2. The typed ClientDisplayName reaches the actual command byte-exactly,
...
//    Client, revision, display name, Operation, actor, contract and time. Each
...
//    ClientRenameDraft, RenameClientCommand or production validation logic.
...
// 6. All 12 ClientRenameFailure values deliberately thrown by the port remain
...
//    same-Client revision precondition, with every broader edit/UI/service/
//    provider/production field excluded.
// 9. READY and implementation diffs obey their exact two-leaf allowlists and
//    preserve the statuses/content of ClientRenameOperation/tests,
//    ClientProjectDirectory/tests, ClientCoreDetailsData/tests,
//    EditProjectModal, ProjectDetailView, ProjectDetailContainer,
//    ProjectService and its protocol, Project model,
//    ProjectFormValidation/tests, the projects MCP module and update_project.
