...
@Suite("Project Rename Operation Contracts")
struct ProjectRenameOperationTests {
    @Test("Rename changes only current display text for one stable Project revision")
    func typedRenameHasExactScopeAndPrecondition() throws {
        ...
        #expect(command.draft.newDisplayName.rawValue == "North Residence")
        #expect(command.draft.expectedRevision == ExpectedProjectRevision(23))
        ...
        #expect(command.envelope.preconditions == [
            .expectedRevision(subject: command.subject, revision: 23)
        ])
        ...
        #expect(payloadKeys == Set(["projectId", "displayName"]))
        ...
        for forbidden in [
            "clientid", "clientname", "description", "category", "media", "image",
            "archive", "restore", "delete", "merge", "reassign", "invoice",
            "transaction", "item", "space", "field", "collection", "sql"
        ] {
            #expect(!encodedText.contains(forbidden))
        }
    }

    @Test("Canonical Project rename evidence survives structured offline restart")
    func canonicalRestartPreservesExactEvidence() throws {
        let command = try Self.command()
        let bytes = try OperationContractCodec.encode(command)
        let restored = try OperationContractCodec.decode(RenameProjectCommand.self, from: bytes)

        #expect(restored == command)
        #expect(try OperationContractCodec.encode(restored) == bytes)
        #expect(restored.envelope.operationId.rawValue == "operation-rename-project-north")
        #expect(restored.envelope.accountId.rawValue == "account-project-rename-test")
        #expect(restored.envelope.actorPrincipalId.rawValue == "principal-project-rename-test")
        #expect(restored.envelope.contractVersion.rawValue == "project-rename-v1")
        ...
        let changedName = try Self.command(
            operationID: "operation-rename-project-north-again",
    ...
    @Test("Invalid, rebound, and tampered Project rename evidence fails atomically")
    func invalidAndTamperedEvidenceFailsClosed() throws {
        ...
        let changedContract = try Self.mutate(
            ...
            value: "project-rename-v2"
        )
        #expect(Self.decodeFailure(changedContract) == .draftContractMismatch)
        let changedPayloadProject = try Self.mutate(
            bytes,
            path: ["envelope", "payload", "projectId"],
            value: "project-other"
        )
        #expect(Self.decodeFailure(changedPayloadProject) == .draftPayloadMismatch)
        ...
        #expect(Self.renameFailure {
            try OperationContractCodec.decode(ProjectRenameDraft.self, from: Data("{}".utf8))
        } == .invalidEncodedDraft)
    ...
    @Test("The Project rename reference port reuses shared queued replay semantics")
    func referencePortReusesSharedOperationLifecycle() async throws {
        let command = try Self.command()
        let adapter = JournalProjectRenameAdapter(acceptedAt: Self.t1)
        ...
        do {
            _ = try await adapter.rename(changedProject)
        ...
        do {
            ...
            Issue.record("A reused OperationID accepted changed Project display text")
        } catch let failure as OperationContractFailure {
            #expect(failure == .payloadMismatch(command.envelope.operationId))
        }
        ...
        let failing = FailingProjectRenameAdapter()
    ...
    private static func draft(
        ...
        displayName: String = "North Residence",
        revision: UInt64 = 23,
        capturedAt: Date = t0
    ) throws -> ProjectRenameDraft {
        try ProjectRenameDraft(
            accountId: AccountID(validating: "account-project-rename-test"),
            actorPrincipalId: PrincipalID(validating: "principal-project-rename-test"),
            operationContractVersion: OperationContractVersion(
                validating: "project-rename-v1"
            ),
    ...
    private static func command(
        operationID: String = "operation-rename-project-north",
        projectID: String = "project-north",
        displayName: String = "North Residence",
        revision: UInt64 = 23
    ) throws -> RenameProjectCommand {
        try RenameProjectCommand(
    ...
    private static func decodeFailure(_ data: Data) -> ProjectRenameFailure? {
        renameFailure {
            try OperationContractCodec.decode(RenameProjectCommand.self, from: data)
        }
    }
...
private actor JournalProjectRenameAdapter: ProjectRenaming {
...
private struct FailingProjectRenameAdapter: ProjectRenaming {
    func rename(_ command: RenameProjectCommand) async throws -> OperationReceipt {
