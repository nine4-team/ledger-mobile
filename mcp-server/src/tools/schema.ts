...
const ENTITIES: Record<string, EntitySchema> = {
  ...
  quick_draft_item: {
    ...
    keyFields: [
      ...
      { name: "spaceId", type: "string?", description: "Optional physical space assignment; independent from financial assignment." },
      ...
      { name: "assignmentHint", type: "enum(quickDraftAssignmentHint)", description: "undecided, client_paid, business_paid, or from_inventory. A reversible hint only." },
      { name: "isFromInventory", type: "boolean", description: "User-selected routing marker. True means a project draft should be matched to the business-inventory purchase that acquired it." },
      ...
      { name: "notes", type: "string?", description: "Direct user-authored instructions and context. Read these before acting; they take precedence over inferred metadata and routing markers." },
    ...
    enums: ["quickDraftItemStatus", "quickDraftCaptureContext", "quickDraftAssignmentHint"],
    notes: [
      "Quick draft items are separate from real items; list_items/search_items do not return them.",
      "Always read and follow user-authored notes before acting on a draft. isFromInventory is only a routing marker and must not override notes or a confirmed transaction association.",
