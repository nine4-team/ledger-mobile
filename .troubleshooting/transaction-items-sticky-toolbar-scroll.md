...
- **Verdict:** **Ruled Out** - Approach abandoned in favor of H10

### H10: Replace stickyHeaderIndices with manual sticky implementation
- **Rationale:** The native `stickyHeaderIndices` is fundamentally incompatible with the Screen's padded wrapper layout. Manual sticky gives full control over positioning.
- **Experiment:**
  - Added `onScroll` handler that tracks scroll position vs control bar Y position
  - Added `onLayout` handler to capture control bar's Y position
  - When scrollY >= controlBarY, render a floating absolute-positioned control bar at top
  - Original control bar stays in scroll content (becomes invisible when floating clone appears)
  - Set `zIndex: 10` on floating bar to appear above all content
  - Removed `StickyHeader` wrapper, removed `collapsable={false}`
  - Kept `styles.list: { gap: 10, marginTop: 12 }` for card spacing
- **Evidence:**
  - Commit `2cfa595` - "feat: improve transaction detail screen and fix sticky header"
  - Lines 114-116: State and refs for sticky tracking
  - Lines 1051-1063: Scroll and layout handlers
  - Lines 1064-1094: `renderControlBar()` function
  - Lines 1115-1121: Conditional floating bar + wrapped AppScrollView
  - Lines 1292-1294: Original control bar with `onLayout`
  - Lines 1589-1598: Styles for `scrollContainer` and `floatingControlBar`
- **Verdict:** _awaiting user testing_

## Resolution
- **Current approach:** Manual sticky implementation (H10) - see Investigation Log
- **Files changed:** `app/transactions/[id]/index.tsx`
- **Commit:** `2cfa595` - "feat: improve transaction detail screen and fix sticky header"
- **Status:** _awaiting user testing_
