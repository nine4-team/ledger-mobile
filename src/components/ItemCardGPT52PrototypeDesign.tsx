...
import { CARD_BORDER_WIDTH, CARD_PADDING, getCardBaseStyle, getCardBorderStyle } from '../ui';
...
export type ItemCardGPT52PrototypeDesignProps = {
  sku?: string;
  sourceLabel?: string;
...
const PROTOTYPE_DESCRIPTION = 'ItemCard GPT 5.2 Prototype Design';

export function ItemCardGPT52PrototypeDesign({
  sku,
  sourceLabel,
  locationLabel,
  notes,
  ...
  onPress,
  style,
}: ItemCardGPT52PrototypeDesignProps) {
  const description = PROTOTYPE_DESCRIPTION;
  ...
  const skuSourceRow = useMemo(() => {
    if (!sku && !sourceLabel) return null;
    if (stackedMeta) {
      return (
        <>
          {sourceLabel ? (
    ...
    const parts = [sku ? `SKU: ${sku}` : null, sourceLabel ? `Source: ${sourceLabel}` : null].filter(Boolean);
    return (
      <Text style={[styles.metaText, themed.metaText]} numberOfLines={1}>
        {parts.join(' • ')}
      </Text>
    );
  }, [sku, sourceLabel, stackedMeta, themed.metaText]);
          ...
          <View style={styles.bottomRow}>
            <View style={styles.thumbCol}>
              {thumbnailUri && !thumbnailUri.startsWith('offline://') ? (
                <Image
                  source={{
                    uri:
                      resolveAttachmentUri({ url: thumbnailUri, kind: 'image', contentType: 'image/jpeg' }) ??
                      thumbnailUri,
                  }}
            ...
            <View style={styles.textCol}>
              <View style={styles.metaCol}>
                {priceLabel ? (
                  <Text style={[styles.metaText, themed.metaText]} numberOfLines={1}>
                    <Text style={styles.metaStrong}>Price:</Text> {priceLabel}
                  </Text>
                ) : null}
                {skuSourceRow}
                {locationLabel ? (
                ...
                {notes ? (
                  <Text style={[styles.metaText, themed.metaText]} numberOfLines={3}>
                    <Text style={styles.metaStrong}>Notes:</Text> {notes}
                  </Text>
                ) : null}
              </View>
            </View>
