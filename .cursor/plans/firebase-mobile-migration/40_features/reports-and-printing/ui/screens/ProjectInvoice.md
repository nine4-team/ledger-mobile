...
- View invoice lines:
  - Each line shows canonical title mapping when the transaction is a canonical inventory sale transaction (recommended: `isCanonicalInventorySale === true`; optional fallback: `transactionId` starts with `SALE_`), otherwise shows source.
