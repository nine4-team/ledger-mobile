...
- [ ] **Canonical path**: entitlement snapshot lives at `accounts/{accountId}/billing/entitlements`.
...
- [ ] **Canonical path**: counters live at `accounts/{accountId}/billing/usage`.
...
- [ ] **Membership/role validated**: server-owned create validates account user per `accounts/{accountId}/users/{uid}` (see `10_architecture/security_model.md`).
...
## Offline behavior
- [ ] **Offline allow only when provable**: when offline, gated creates proceed only if cached `billing/entitlements` + cached `billing/usage` prove under-limit.
