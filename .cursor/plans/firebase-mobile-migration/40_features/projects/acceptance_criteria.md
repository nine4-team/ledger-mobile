...
- [ ] **List UI**: Projects render in a grid with name + client name + budget preview, and optional main image.  
  Budget preview shows **Overall Budget + pinned budget categories** (per-user per-project pins; same subset as the collapsed Budget module).  
...
- [ ] **Pinned categories seeded for new projects (required)**: on successful project creation, the system seeds pinned budget categories so **“Furnishings” is pinned by default** for the creator’s project preferences (`ProjectPreferences.pinnedBudgetCategoryIds`).  
  Note: pins are per-user per-project; for other users, create `ProjectPreferences` lazily when a UI surface needs it (Projects list preview / Budget module), if missing.  
...
- [ ] **Route parity**: Project shell is mounted at `/project/:projectId/*` and exposes tabs for `items`, `transactions`, `spaces`, `accounting`.  
